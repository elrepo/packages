...
# http://us.download.nvidia.com/XFree86/Linux-x86_64/470.129.06/README/minimumrequirements.html
...
Requires:	nvidia-470xx-kmod = %{?epoch:%{epoch}:}%{version}
Requires(post):	nvidia-470xx-kmod = %{?epoch:%{epoch}:}%{version}
...
%{__install} -p -m 0644 %{SOURCE2} $RPM_BUILD_ROOT%{_prefix}/lib/nvidia/alternate-install-present
