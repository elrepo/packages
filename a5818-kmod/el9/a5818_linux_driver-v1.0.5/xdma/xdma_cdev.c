...
static const char * const devnode_names[] = {
	...
	XDMA_NODE_NAME "%d_vme_events_%d", // CAEN A5818
...
static int create_sys_device(struct xdma_cdev *xcdev, enum cdev_type type)
{
	...
	if (type == CHAR_EVENTS || type == CHAR_VME_EVENTS)
...
void xpdev_destroy_interfaces(struct xdma_pci_dev *xpdev)
{
	...
	if (xpdev_flag_test(xpdev, XDF_CDEV_VME_EVENT)) {
		for (i = 0; i < xpdev->vme_user_max; i++) {
			rv = destroy_xcdev(&xpdev->vme_events_cdev[i]);
			if (rv < 0)
				pr_err("Failed to destroy cdev VME event %d error 0x%x\n",
					i, rv);
		}
	}
...
int xpdev_create_interfaces(struct xdma_pci_dev *xpdev)
{
	...
	/* CAEN A5818: initialize VME events character device */
	for (i = 0; i < xpdev->vme_user_max; i++) {
		rv = create_xcdev(xpdev, &xpdev->vme_events_cdev[i], i, NULL,
			CHAR_VME_EVENTS);
		if (rv < 0) {
			pr_err("create char VME event %d failed, %d.\n", i, rv);
			goto fail;
		}
	}
	xpdev_flag_set(xpdev, XDF_CDEV_VME_EVENT);
...
static char *caen_a5818_devnode(struct device *dev, umode_t *mode)
#else
static char *caen_a5818_devnode(const struct device *dev, umode_t *mode)
#endif
#else
