...
#define DRV_MODULE_DESC		"Xilinx XDMA Reference Driver, modified by CAEN SpA for A5818"
...
static struct xdma_pci_dev *xpdev_alloc(struct pci_dev *pdev)
{
	...
	xpdev->vme_user_max = MAX_VME_USER_IRQ;
...
static int probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	hndl = xdma_device_open(DRV_MODULE_NAME, pdev, &xpdev->user_max, &xpdev->vme_user_max,
	...
	if (xpdev->vme_user_max > MAX_VME_USER_IRQ) {
		pr_err("Maximum VME users limit reached\n");
		rv = -EINVAL;
		goto err_out;
	}
	...
	pr_info("%s xdma%d, pdev 0x%p, xdev 0x%p, 0x%p, usr %d, vmeusr %d, ch %d,%d.\n",
		dev_name(&pdev->dev), xdev->idx, pdev, xpdev, xdev,
		xpdev->user_max, xpdev->vme_user_max, xpdev->h2c_channel_max,
	...
	xpdev->proc_dir = xdma_register_proc(xpdev);
...
static void remove_one(struct pci_dev *pdev)
{
	...
	xdma_unregister_proc(xpdev->proc_dir);
