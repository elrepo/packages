...
// CAEN A5818
static irqreturn_t vme_user_irq_service(int irq, struct xdma_user_irq *user_irq)
{
	struct xdma_dev* const xdev = user_irq->xdev;
	const int link_number = user_irq->user_idx;
	int conet_node;

	char* user_link_offset;
    const ptrdiff_t link_number_offset = L1_TXFIFO_OFFSET - L0_TXFIFO_OFFSET; // should be 0x1000
	const ptrdiff_t irqstat0_offset = L0_IRQSTAT_0_OFFSET;
	const ptrdiff_t irqmask0_offset = L0_IRQMASK_0_OFFSET;
	const ptrdiff_t irqstat1_offset = L0_IRQSTAT_1_OFFSET;
	const ptrdiff_t irqmask1_offset = L0_IRQMASK_1_OFFSET;
	const ptrdiff_t ioctl_offset = L0_IOCTL_OFFSET;
	const u32 vintdis = 0x8;

	u32 irqstat0, irqstat1;
	u32 irqmask0, irqmask1;
	u32 ioctl;

	if (link_number >= 4) {
		pr_err("Invalid link_number\n");
		return IRQ_NONE;
	}
	...
	user_link_offset = (char*)xdev->bar[xdev->user_bar_idx] + (link_number_offset * link_number);

	// read IRQSTAT0 and clear IRQMASK0
	irqstat0 = ioread32(user_link_offset + irqstat0_offset);
	irqmask0 = ioread32(user_link_offset + irqmask0_offset);
	irqmask0 &= ~irqstat0;
	iowrite32(irqmask0, user_link_offset + irqmask0_offset);

	// read IRQSTAT1 and clear IRQMASK1
	irqstat1 = ioread32(user_link_offset + irqstat1_offset);
	irqmask1 = ioread32(user_link_offset + irqmask1_offset);
	irqmask1 &= ~irqstat1;
	iowrite32(irqmask1, user_link_offset + irqmask1_offset);

	dbg_irq("irqstat0=0x%08lX irqstat1=0x%08lX", (unsigned long)irqstat0, (unsigned long)irqstat1);

	// disable interrupts on IOCTL
	ioctl = ioread32(user_link_offset + ioctl_offset);
	ioctl |= vintdis;
	iowrite32(ioctl, user_link_offset + ioctl_offset);

	// Loop over conet nodes
	for (conet_node = 0; conet_node < 8; ++conet_node) {

		struct xdma_vme_user_irq* vme_user_irq;
		u32 user_mask;

		const u32 irqstat = conet_node < 4 ? irqstat0 : irqstat1;
		const u8 irq_level_mask = (u8)(irqstat >> (conet_node % 4) * 8);

		if (irq_level_mask == 0)
			continue;

		vme_user_irq = &xdev->vme_user_irq[link_number * 8 + conet_node];

		/*
         * See comment on ioctl_wait for details on the operation order.
         */

		// 1. set events_irq, checked by ioctl before waiting for event.
        atomic_set(&vme_user_irq->last_irq, irq_level_mask);

		// 2. ensure events_irq is set before checking user_mask
		mb();

		// 3. user_mask != 0 only if there is a pending call to ioctl
		user_mask = vme_user_irq->user_mask;

		if (!(irq_level_mask & user_mask))
			continue;

		wake_up_interruptible(&(vme_user_irq->events_wq));

	}

	// reenable interrupts on IOCTL
	ioctl = ioread32(user_link_offset + ioctl_offset);
	ioctl &= ~vintdis;
	iowrite32(ioctl, user_link_offset + ioctl_offset);
...
static irqreturn_t user_irq_service(int irq, struct xdma_user_irq *user_irq)
{
	...
	vme_user_irq_service(irq, user_irq); // CAEN A5818
...
static int map_bars(struct xdma_dev *xdev, struct pci_dev *dev)
{
	...
	if (rv <= 0) {
		pr_info("%s, map user bar %d failed, %d.\n",
...
static struct xdma_dev *alloc_dev_instance(struct pci_dev *pdev)
{
	...
	/* CAEN A5818: Set up data user IRQ data structures */
	for (i = 0; i < 32; i++) {
		xdev->vme_user_irq[i].xdev = xdev;
		init_waitqueue_head(&xdev->vme_user_irq[i].events_wq);
		xdev->vme_user_irq[i].user_idx = i; /* 0 based */
		xdev->vme_user_irq[i].user_mask = 0;
		atomic_set(&xdev->vme_user_irq[i].last_irq, 0);
	}
...
void *xdma_device_open(const char *mname, struct pci_dev *pdev, int *user_max, int* vme_user_max,
		       int *h2c_channel_max, int *c2h_channel_max)
{
	...
	xdev->vme_user_max = *vme_user_max;
	...
	if (xdev->vme_user_max == 0 || xdev->vme_user_max > MAX_VME_USER_IRQ)
		xdev->vme_user_max = MAX_VME_USER_IRQ;
	...
	*vme_user_max = xdev->vme_user_max;
