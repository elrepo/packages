...
# Sources
Source0:	ftp://ftp.ts.fujitsu.com/pub/Mainboard-OEM-Sales/Services/Software&Tools/Linux_SystemMonitoring&Watchdog&GPIO/%{kmod_name}-module_%{version}.zip
...
%{__install} -m 0644 assets/*.conf %{buildroot}%{_defaultdocdir}/kmod-%{kmod_name}-%{version}/
%{__install} -m 0644 assets/fts-teutates-D3417-B.conf %{buildroot}%{_defaultdocdir}/kmod-%{kmod_name}-%{version}/fts-teutates-D3417-B1.conf
%{__install} -m 0644 doc/* %{buildroot}%{_defaultdocdir}/kmod-%{kmod_name}-%{version}/
%{__install} -m 0644 test/* %{buildroot}%{_defaultdocdir}/kmod-%{kmod_name}-%{version}/
# prepare for ghost file
%{__mkdir} -p %{buildroot}%{_sysconfdir}/sensors.d/
touch %{buildroot}%{_sysconfdir}/sensors.d/fts-teutates.conf
...
BOARDVERSION=$(dmidecode -t system | grep "Product Name" | sed -e "s#.*Product Name: ##")
CONFIGFILE=%{_defaultdocdir}/kmod-%{kmod_name}-%{version}/fts-teutates-${BOARDVERSION}.conf
if [ -f $CONFIGFILE -a ! -s /etc/sensors.d/fts-teutates.conf  ]; then
	%{__cp} $CONFIGFILE /etc/sensors.d/fts-teutates.conf
fi
...
%changelog
* Tue Nov 08 2022 Philip J Perry <phil@elrepo.org> 20190927-7
- Rebuilt for RHEL 8.7

* Tue May 10 2022 Philip J Perry <phil@elrepo.org> 20190927-6
- Rebuilt for RHEL 8.6
- Fix SB-signing issue caused by /usr/lib/rpm/brp-strip
  [https://bugzilla.redhat.com/show_bug.cgi?id=1967291]

* Tue May 18 2021 Philip J Perry <phil@elrepo.org> 20190927-5
- Rebuilt for RHEL8.4
...
* Sun Dec 08 2019 Jens Kuehnel <elrepo-package@jens.kuehnel.org> - 20190927-1
- copy template file to sensors.d if not done yet
- use template depending on installed motherboard

* Sun Dec 08 2019 Jens Kuehnel <elrepo-package@jens.kuehnel.org> 20190927-0
