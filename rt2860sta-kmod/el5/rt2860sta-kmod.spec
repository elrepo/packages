...
URL:	 http://www.ralinktech.com.tw/data/drivers/2010_07_16_RT2860_Linux_STA_v2.4.0.0.tgz
...
%prep
%setup -q -c -T -a 0
pushd 2010_07_16_RT2860_Linux_STA_v2.4.0.0
...
for kvariant in %{kvariants} ; do
    %{__cp} -a 2010_07_16_RT2860_Linux_STA_v2.4.0.0 _kmod_build_$kvariant
done
%{__cp} -a %{SOURCE5} .
...
for kvariant in %{kvariants} ; do
    ...
    %{__install} os/linux/rt2860sta.ko %{buildroot}/lib/modules/%{kversion}${kvariant}/extra/%{kmod_name}/
...
%changelog
* Mon May 02 2011 Philip J Perry <phil@elrepo.org> - 2.4.0.0-1.el5.elrepo
- Update to version 2.4.0.0.

* Sun Aug 30 2009 Philip J Perry <phil@elrepo.org> - 2.1.2.0-1.el5.elrepo
- Initial build of the kmod package.
- Patch makefile to build against kernel variants. [rt2860-makefile.patch]
- Patch config.mk to build for wpa_supplicant support. [rt2860-config-wpa-supplicant.patch]
- Fix unsigned long int compile warnings on x86_64. [rt2860-unsigned-long-int-warning.patch]
