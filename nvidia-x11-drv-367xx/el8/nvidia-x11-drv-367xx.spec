...
# https://download.nvidia.com/XFree86/Linux-x86_64/367.134/README/minimumrequirements.html
...
Source3:	90-nvidia-367xx.preset
Source4:	10_nvidia.json
Source5:	10_nvidia_wayland.json
...
Requires:	kmod-nvidia-367xx = %{?epoch:%{epoch}:}%{version}
Requires(post):	kmod-nvidia-367xx = %{?epoch:%{epoch}:}%{version}
...
%{__install} -p -m 0644 %{SOURCE4} $RPM_BUILD_ROOT%{_datadir}/glvnd/egl_vendor.d/
%{__mkdir_p} $RPM_BUILD_ROOT%{_datadir}/egl/egl_external_platform.d
%{__install} -p -m 0644 %{SOURCE5} $RPM_BUILD_ROOT%{_datadir}/egl/egl_external_platform.d/
%endif
