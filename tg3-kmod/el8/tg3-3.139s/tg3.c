...
#if defined BCM_HAS_PTP_AUX_WORK
...
static void tg3_napi_init(struct tg3 *tp)
{
	...
	__netif_napi_add(tp->dev, &tp->napi[0].napi, tg3_poll, 64);
	for (i = 1; i < tp->irq_cnt; i++)
		__netif_napi_add(tp->dev, &tp->napi[i].napi, tg3_poll_msix, 64);
#else
...
abort_lso:
	...
	if ((unlikely(skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP)) &&
		...
		if (!tp->pre_tx_ts) {
			...
			tp->pre_tx_ts = tr32(TG3_TX_TSTAMP_LSB);
			tp->pre_tx_ts |= (u64)tr32(TG3_TX_TSTAMP_MSB) << 32;
		}
		tg3_full_unlock(tp);
	}
...
static void tg3_ptp_task(struct work_struct *work)
#else
static void tg3_ptp_task(void *_data)
#endif
{
#ifdef BCM_HAS_NEW_INIT_WORK
	struct tg3 *tp = container_of(work, struct tg3, ptp_task);
#else
	struct tg3 *tp = _data;
#endif
	struct skb_shared_hwtstamps timestamp;
...
static void __devinit tg3_get_eeprom_hw_cfg(struct tg3 *tp)
{
	...
	if (val == NIC_SRAM_DATA_SIG_MAGIC) {
		...
		if (nic_cfg & NIC_SRAM_DATA_CFG_EEPROM_WP) {
			...
			case PCI_VENDOR_ID_ARIMA:
				if (tp->pdev->subsystem_device == 0x205a ||
...
static int __devinit tg3_get_device_address(struct tg3 *tp, u8 *addr)
{
	struct net_device *dev = tp->dev;
	...
	memcpy(dev->perm_addr, addr, dev->addr_len);
#endif
	return 0;
}
...
#if !defined BCM_HAS_PTP_AUX_WORK
	INIT_WORK(&tp->ptp_task, tg3_ptp_task);
#endif
#else
	INIT_WORK(&tp->reset_task, tg3_reset_task, tp);
#if !defined BCM_HAS_PTP_AUX_WORK
	INIT_WORK(&tp->ptp_task, tg3_ptp_task, tp);
#endif
#endif /* BCM_HAS_NEW_INIT_WORK */
