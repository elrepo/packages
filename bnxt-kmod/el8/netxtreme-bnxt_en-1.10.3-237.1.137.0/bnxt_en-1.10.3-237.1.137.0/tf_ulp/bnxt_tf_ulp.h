...
/* DSCP remap definitions for insert operation */
#define BNXT_ULP_DSCP_REMAP_ROWS			64
#define BNXT_ULP_DSCP_MAX_VFS_PER_COLUMN		48
...
struct bnxt_ulp_dscp_remap {
	...
	u32				*dscp_global_cfg;
	struct bnxt_dscp_remap_vf	dscp_remap_vf[BNXT_ULP_DSCP_REMAP_ROWS][MTR_PROF_CLR_MAX];
};
...
#else
static inline int bnxt_ulp_pf_veb_flow_create(struct bnxt *bp) { return 0; }
static inline int bnxt_ulp_pf_veb_flow_delete(struct bnxt *bp) { return 0; }
static inline struct bnxt_ulp_context	*
bnxt_ulp_bp_ptr2_cntxt_get(struct bnxt *bp) { return NULL; }
