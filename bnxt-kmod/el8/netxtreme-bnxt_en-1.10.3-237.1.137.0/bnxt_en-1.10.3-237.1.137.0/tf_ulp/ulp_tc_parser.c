					  ...
					  u32 value, u32 set_flag, bool hw_wr)
{
	...
	size = sizeof(u32) * BNXT_ULP_DSCP_REMAP_ROWS;
	parms.dir = dir,
					...
					struct bnxt_ulp_dscp_remap *dscp_remap)
{
	...
	for (vf_id = 0; vf_id < BNXT_ULP_DSCP_REMAP_ROWS; vf_id++) {
				     ...
				     int *row_idx, bool *update_glb_cfg)
{
	...
	/* If dscp insert capability flag is enabled:
	 * VFs 0-47 map to yellow and VFs 48-95 map to
	 * green.
	 */
	if (BNXT_ULP_DSCP_INSERT_CAP(dscp_remap)) {
		if (vf_id < BNXT_ULP_DSCP_MAX_VFS_PER_COLUMN) {
			row = vf_id;
			color = MTR_PROF_CLR_YELLOW;

		} else {
			row = vf_id - BNXT_ULP_DSCP_MAX_VFS_PER_COLUMN;
			color = MTR_PROF_CLR_GREEN;
		}

		if (row >= BNXT_ULP_DSCP_REMAP_ROWS) {
			netdev_dbg(bp->dev, "%s: dscp remap row error\n", __func__);
			return -EINVAL;
		}

		if (bnxt_tc_is_row_reserved(dscp_remap, row))
			return -EINVAL;

		dscp_remap_vf = dscp_remap->dscp_remap_vf[row];
		if (dscp_remap_vf[color].dscp_remap_val == dscp_val) {
			netdev_dbg(bp->dev, "%s: row(%d) color(%d) remap(0x%x) already matches value set\n",
				   __func__, row, color, dscp_val);
			*update_glb_cfg = false;
			goto done;
		}

		if (dscp_remap_vf[color].dscp_remap_val == BNXT_ULP_DSCP_INVALID) {
			netdev_dbg(bp->dev, "%s: row(%d) color(%d) free\n", __func__, row, color);
		} else {
			netdev_dbg(bp->dev, "%s: no space left row(%d) color(%d)\n", __func__, row,
				   color);
			return -ENOSPC;
		}
	} else {
	...
	netdev_dbg(bp->dev, "%s: row(%d) color(%d) set to (0x%x)\n", __func__, row, color,
		   dscp_val);
						...
						*dscp_remap)
{
	...
	if (!BNXT_ULP_DSCP_INSERT_CAP(dscp_remap)) {
		...
		vf_id = BNXT_ULP_DSCP_REMAP_ROWS - 1; /* for hw_wr below */
	} else {
		for (i = 0; i < ARRAY_SIZE(bnxt_tc_dscp_resv_rows); i++) {
