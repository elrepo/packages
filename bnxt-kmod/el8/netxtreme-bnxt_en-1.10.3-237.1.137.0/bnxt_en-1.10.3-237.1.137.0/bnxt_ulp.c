...
int bnxt_ulp_cp_rings_in_use(struct bnxt *bp)
{
	if (bp->edev_rdma)
		return 2 * bp->edev_rdma->ulp_num_ts_qps;
	return 0;
}

int bnxt_ulp_rx_rings_in_use(struct bnxt *bp)
{
	if (bp->edev_rdma)
		return 2 * bp->edev_rdma->ulp_num_ts_qps;
	return 0;
...
int bnxt_ulp_tx_rings_in_use(struct bnxt *bp)
{
	if (bp->edev_rdma)
		return bp->edev_rdma->ulp_num_ts_qps;
	return 0;
}

int bnxt_ulp_vnics_in_use(struct bnxt *bp)
{
	if (bp->edev_rdma)
		return bp->edev_rdma->ulp_num_ts_qps;
	return 0;
...
{
	if (bp->edev_rdma)
		bp->edev_rdma->ulp_num_ts_qps = num;
}
