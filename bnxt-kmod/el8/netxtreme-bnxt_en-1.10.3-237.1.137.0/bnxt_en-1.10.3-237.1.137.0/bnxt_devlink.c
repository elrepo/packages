...
enum bnxt_dl_param_id {
	...
	BNXT_DEVLINK_PARAM_ID_PREALLOC_KIDS,
};
...
static int bnxt_dl_prealloc_kid_param_get(struct devlink *dl, u32 id,
					  struct devlink_param_gset_ctx *ctx)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	struct bnxt_tls_info *tls_info;

	if (!bp->quic_info && !bp->ktls_info)
		return -EOPNOTSUPP;

	if (bp->ktls_info)
		tls_info = bp->ktls_info;
	else
		tls_info = bp->quic_info;

	ctx->val.vu32 = tls_info->tck.total_alloc;

	return 0;
}

static int bnxt_dl_prealloc_kid_param_set(struct devlink *dl, u32 id,
					  struct devlink_param_gset_ctx *ctx,
					  struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (bp->ktls_info) {
		tls_info = bp->ktls_info;
		tls_type = BNXT_CRYPTO_TYPE_KTLS;
	} else {
		tls_info = bp->quic_info;
		tls_type = BNXT_CRYPTO_TYPE_QUIC;
	}
	...
	if (ctx->val.vu32 <= kctx->total_alloc)
		goto done;

	needed = ctx->val.vu32 - kctx->total_alloc;
	...
	for (i = 0; i < pre_alloc_kids_num_batch; i++) {
		...
		if (rc) {
			netdev_warn(bp->dev,
				    "Preallocated %d TX KIDs only requested: %d\n",
				    i * BNXT_KID_BATCH_SIZE, ctx->val.vu32);
...
static int bnxt_dl_prealloc_kid_param_validate(struct devlink *dl, u32 id,
					       union devlink_param_value val,
					       struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!val.vu32) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "prealloc_tx_kids value must be greater than zero");
		return -EINVAL;
	}

	tls_info = bp->ktls_info ?: bp->quic_info;
	kctx = &tls_info->tck;

	if (val.vu32 <= kctx->total_alloc) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested number of KIDs are already allocated");
		return -EINVAL;
	}

	if (val.vu32 > BNXT_MAX_TX_CRYPTO_KEYS) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested KIDs to prealloc exceed capacity");
		return -ENOSPC;
	}
...
static const struct devlink_param bnxt_dl_params[] = {
	...
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_TRUFLOW,
			     ...
			     NULL),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_PREALLOC_KIDS,
			     "prealloc_tx_kids", DEVLINK_PARAM_TYPE_U32,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_prealloc_kid_param_get, bnxt_dl_prealloc_kid_param_set,
			     bnxt_dl_prealloc_kid_param_validate),
#ifdef HAVE_REMOTE_DEV_RESET
