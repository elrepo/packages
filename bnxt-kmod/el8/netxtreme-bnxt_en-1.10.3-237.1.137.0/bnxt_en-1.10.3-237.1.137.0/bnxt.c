...
int bnxt_total_tx_rings(struct bnxt *bp)
{
	...
	return tx + bnxt_ulp_tx_rings_in_use(bp);
}
...
static int bnxt_cp_rings_in_use(struct bnxt *bp)
{
	...
	cp += bnxt_ulp_cp_rings_in_use(bp);
...
{
	u16 nr_ulp_vnics = bnxt_ulp_vnics_in_use(bp);
...
int bnxt_total_rx_rings(struct bnxt *bp)
{
	...
	if (bp->flags & BNXT_FLAG_AGG_RINGS)
		rx <<= 1;
	return rx + bnxt_ulp_rx_rings_in_use(bp);
}
...
static bool bnxt_need_reserve_rings(struct bnxt *bp)
{
	...
	hwr.rx += bnxt_ulp_rx_rings_in_use(bp);
...
static bool bnxt_trim_ulp_rings(struct bnxt *bp,
				struct bnxt_hw_rings *hwr)
{
	if (!bnxt_get_ulp_num_rings(bp))
		return true;

	if (hwr->tx < bnxt_ulp_tx_rings_in_use(bp) ||
	    hwr->rx < bnxt_ulp_rx_rings_in_use(bp) ||
	    hwr->cp_p5 < bnxt_ulp_cp_rings_in_use(bp) ||
	    hwr->vnic < bnxt_ulp_vnics_in_use(bp))
		return false;

	hwr->tx -= bnxt_ulp_tx_rings_in_use(bp);
	hwr->rx -= bnxt_ulp_rx_rings_in_use(bp);
	hwr->cp_p5 -= bnxt_ulp_cp_rings_in_use(bp);
	hwr->vnic -= bnxt_ulp_vnics_in_use(bp);
	return true;
...
static int bnxt_hwrm_monitor_dev_health(struct bnxt *bp, u32 *status)
{
				...
				int *max_cp)
{
	...
	*max_tx -= bnxt_ulp_tx_rings_in_use(bp);
	*max_rx = hw_resc->max_rx_rings;
	*max_rx -= bnxt_ulp_rx_rings_in_use(bp);
