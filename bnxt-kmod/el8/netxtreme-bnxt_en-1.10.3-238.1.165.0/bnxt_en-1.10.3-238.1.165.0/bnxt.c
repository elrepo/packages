...
#if defined(HAVE_FWCTL_CORE)
#ifdef HAVE_MODULE_IMPORT_NS_STRING
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Broadcom NetXtreme-C/E network driver");
...
static bool bnxt_vf_pciid(enum board_idx idx)
{
	return (idx == NETXTREME_C_VF || idx == NETXTREME_E_VF ||
		idx == NETXTREME_C_VF_HV || idx == NETXTREME_E_VF_HV ||
		idx == NETXTREME_E_P5_VF || idx == NETXTREME_E_P5_VF_HV ||
		idx == NETXTREME_E_P7_VF || idx == NETXTREME_E_P7_VF_HV ||
		idx == NETXTREME_E_P8_VF);
}
...
static u8 bnxt_generate_txtimed_bd(struct bnxt *bp, struct sk_buff *skb,
				   struct bnxt_tx_ring_info *txr, u16 *prod)
{
	struct tx_bd_timed *tx_bd_txtime;
	u32 sotxtm_flags;
	s64 txtime_ns;
	...
	*prod = NEXT_TX(*prod);
	...
	tx_bd_txtime = (struct tx_bd_timed *)
...
static u8 bnxt_generate_ttxp_bd(struct bnxt *bp,
				struct bnxt_tx_ring_info *txr, u16 *prod)
{
	u32 flags = TX_BD_TYPE_TIMEDTX_BD;
	struct tx_bd_timed *tx_bd_txtime;
	...
	*prod = NEXT_TX(*prod);
	tx_bd_txtime = (struct tx_bd_timed *)
		&txr->tx_desc_ring[TX_RING(bp, *prod)][TX_IDX(*prod)];
	if (txr->pacing_rate)
		flags |= TX_BD_FLAGS_KIND_PACE;
	tx_bd_txtime->tx_bd_len_flags_type = cpu_to_le32(flags);
	tx_bd_txtime->rate = cpu_to_le32(txr->pacing_rate);
	txr->pacing_enabled = !txr->pacing_enabled;
	return 1;
}

static inline bool is_roce_pkt(struct sk_buff *skb)
{
	if (cpu_to_be16(udp_hdr(skb)->dest) == ROCE_V2_UDP_DPORT) {
			      ...
			      struct sk_buff *skb, __le32 lflags, u32 kid)
{
	...
	prod = txr->tx_prod;
#if defined(HAVE_IPV6_BIG_TCP) && !defined(HAVE_IPV6_PAYLOAD_LEN)
	...
	if (unlikely(BNXT_SRIOV_DSCP_INSERT_CAP(bp) && BNXT_VF(bp))) {
		u32 dscp = (bp->vf.fw_fid - BNXT_FIRST_VF_FID) % BNXT_ULP_DSCP_MAX_VFS_PER_COLUMN;
...
#if defined(HAVE_ETF_QOPT_OFFLOAD)
	if (txr->etf_enabled)
		extra_bds = bnxt_generate_txtimed_bd(bp, skb, txr, &prod);
	else if (BNXT_TTXP_PENDING(txr))
#else
	if (BNXT_TTXP_PENDING(txr))
#endif
		extra_bds = bnxt_generate_ttxp_bd(bp, txr, &prod);

	tx_buf->extra_bds = extra_bds;
	total_bds = BNXT_TX_BD_LONG_CNT + extra_bds + last_frag;

	if (unlikely(READ_ONCE(bp->soft_roce_icrc_en)) && is_roce_pkt(skb))
		txbd1->tx_bd_hsize_lflags |= cpu_to_le32(TX_BD_FLAGS_ROCE_CRC);

	flags |= (len << TX_BD_LEN_SHIFT) | TX_BD_TYPE_LONG_TX_BD |
		 TX_BD_CNT(total_bds);
	txbd->tx_bd_opaque = SET_TX_OPAQUE(bp, txr, prod0, total_bds);
...
static void bnxt_bs_trace_init(struct bnxt *bp, struct bnxt_ctx_mem_type *ctxm)
{
	...
	if (ctxm->instance_bmap > 1 || ctxm->max_inst_cnt > 1)
		return;
	...
	if (pages > MAX_CTX_PAGES) {
		...
		rmem_pg_tbl = &ctx_pg[0].ctx_pg_tbl[last_pg_directory]->ring_mem;
...
#define BNXT_EVENT_L2_TX_RING_ID(data2)						\
	((data2) &								\
	 ASYNC_ERROR_REPORT_L2_TX_RING_EVENT_DATA2_TX_RING_ID_MASK)

#define BNXT_EVENT_L2_TX_ERR_REASON(data1)					\
	(((data1) &								\
	 ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_MASK) >>				\
	 ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_SFT)

static char *bnxt_decode_l2_tx_ring_error(u32 data1)
{
	char *err_reason_str;

	switch (data1 & ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_MASK) {
	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_NO_ERR:
		err_reason_str = "No Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_CMP_ERR:
		err_reason_str = "Completion Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_BD_FORMAT_ERR:
		err_reason_str = "BD format Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_BD_LENGTH_ERR:
		err_reason_str = "BD length Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_CQ_LOAD_ERR:
		err_reason_str = "CQ load Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_SERV_TYPE_ERR:
		err_reason_str = "Server type Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_TX_PCI_ERR:
		err_reason_str = "PCI Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_PROD_BD_MSMTCH_ERR:
		err_reason_str = "Prod BD mismatch Error";
		break;

	case ASYNC_ERROR_REPORT_L2_TX_RING_ERR_RSN_COS_OOR_ERR:
		err_reason_str = "Cos OOR Error";
		break;
...
static bool bnxt_event_error_report(struct bnxt *bp, u32 data1, u32 data2)
{
	...
	case ASYNC_EVENT_CMPL_ERROR_REPORT_BASE_EVENT_DATA1_ERROR_TYPE_DOORBELL_DROP_THRESHOLD:
		...
		break;
	case ASYNC_EVENT_CMPL_ERROR_REPORT_BASE_EVENT_DATA1_ERROR_TYPE_DB_DROP:
		dev_warn_ratelimited(&bp->dev->dev,
				     "Async doorbell drop event data1 = 0x%08x data2 = 0x%08x",
				     data1, data2);
		break;
	...
	case ASYNC_EVENT_CMPL_ERROR_REPORT_BASE_EVENT_DATA1_ERROR_TYPE_L2_TX_RING: {
		netdev_warn(bp->dev,
			    "Tx ring error detected, reason: %s, ring ID: 0x%04x\n",
			    bnxt_decode_l2_tx_ring_error(data1),
			    (u32)BNXT_EVENT_L2_TX_RING_ID(data2));
		break;
	}
...
static int bnxt_reset_notify(struct bnxt *bp,
			     struct hwrm_async_event_cmpl *cmpl)
{
	if (!bp->fw_health)
		return -EOPNOTSUPP;

	if (test_and_set_bit(BNXT_STATE_RESET_NOTIFY_BEGIN, &bp->state)) {
		netdev_warn(bp->dev, "Reset notify handling is already in progress\n");
		return -EALREADY;
	}
...
static void bnxt_reset_notify_done(struct bnxt *bp)
{
	clear_bit(BNXT_STATE_RESET_NOTIFY_BEGIN, &bp->state);
}
				       ...
				       int idx)
{
	...
	for (i = 0; i < max_idx;) {
		...
		if (BNXT_IS_TX_XDP_RDIR_RING(bp, idx) &&
...
static int bnxt_alloc_tx_rings(struct bnxt *bp)
{
	...
	for (i = 0, j = 0; i < bp->tx_nr_rings; i++) {
		...
		if (BNXT_RING_TO_TC_OFF(bp, i) == (bp->tx_nr_rings_per_tc - 1))
			j++;
		if (ttxp)
			txr->pacing_rate = ttxp->ring_rates[i - bp->tx_nr_rings_xdp] / 8;
	}
	return 0;
}
...
static int bnxt_get_tx_ring_idx(struct bnxt *bp, int napi_idx, int off,
				int inst)
{
	int n = BNXT_TC_TO_RING_BASE(bp, inst);
	int base = napi_idx;

	if (napi_idx >= BNXT_TX_XDP_RINGS_PER_GRP(bp)) {
		napi_idx -= BNXT_TX_XDP_RINGS_PER_GRP(bp);
		base = bp->tx_nr_rings_xdp + napi_idx - off;
	}
	return base + n;
}
...
static int bnxt_alloc_cp_rings(struct bnxt *bp, bool irq_re_init)
{
	...
	if (!tcs)
		tcs = 1;
	ulp_msix = bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
	for (i = 0; i < bp->cp_nr_rings; i++) {
		struct bnxt_napi *bnapi = bp->bnapi[i];
		struct bnxt_cp_ring_info *cpr, *cpr2;
		struct bnxt_ring_struct *ring;
		int cp_count = 0, k, off = 0;
		int rx = 0, tx = 0;
		...
		if (i < BNXT_TX_XDP_RINGS_PER_GRP(bp)) {
			cp_count += BNXT_XDP_TX_PER_NAPI;
			tx = BNXT_XDP_TX_PER_NAPI;
		} else if ((sh && i < bp->tx_nr_rings) ||
			 (!sh && i >= bp->rx_nr_rings)) {
			cp_count += tcs;
			tx = tcs;
			off = sh ? 0 : bp->rx_nr_rings;
		}
		...
		for (k = 0; k < cp_count; k++) {
			...
			if (k < rx) {
				...
				n = bnxt_get_tx_ring_idx(bp, i, off, tc);
...
static void bnxt_free_vnic_attributes(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->nr_vnics; i++) {
		...
		kfree(vnic->l2_filters);
		vnic->l2_filters = NULL;
...
static int bnxt_alloc_vnic_attributes(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->nr_vnics; i++) {
		...
		if (vnic->flags & BNXT_VNIC_UCAST_FLAG) {
			int mem_size = (bp->max_uc_fltrs - 1) * ETH_ALEN;
			...
			vnic->l2_filters = kcalloc(bp->max_uc_fltrs,
						   sizeof(*vnic->l2_filters),
						   GFP_KERNEL);
			if (!vnic->l2_filters) {
				rc = -ENOMEM;
				goto out;
	...
	if (irq_re_init) {
		...
		for (i = 0; i < bp->tx_nr_rings; i++) {
			...
			if (i >= bp->tx_nr_rings_xdp) {
				...
				int k = i % BNXT_TX_XDP_RDIR_RING_BASE(bp);

				bnapi2 = bp->bnapi[k];
				if (BNXT_IS_TX_XDP_RING(bp, i)) {
					txr->tx_napi_idx = BNXT_XDP_TX_IDX;
					...
					txr->tx_napi_idx = BNXT_XDP_TX_RDIR_IDX;
				}
				bnapi2->tx_ring[txr->tx_napi_idx] = txr;
			}
			txr->bnapi = bnapi2;
...
static void bnxt_set_txr_etf_bmap(struct bnxt *bp)
{
	...
	/* ETF bitmap is indexed without xdp */
	tx_count = bp->tx_nr_rings - bp->tx_nr_rings_xdp;
				    ...
				    u32 ring_type, u32 map_index)
{
	...
	case HWRM_RING_ALLOC_CMPL:
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
			...
			hw_coal = (BNXT_NQ_HDL_TYPE(ring->handle) == BNXT_NQ_HDL_TYPE_TX) ?
						    &bp->tx_coal : &bp->rx_coal;
			req->flags |= cpu_to_le16(hw_coal->ring_alloc_flags);
		} else {
...
{
	u32 idx = map_idx - bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
	u32 avail;
...
int bnxt_total_tx_rings(struct bnxt *bp)
{
	...
	tx = bp->tx_nr_rings + bnxt_mpc_tx_rings_in_use(bp);
	return tx + bnxt_get_ulp_resc(bp, BNXT_ULP_RES_TX);
}
...
int bnxt_hwrm_reserve_rings(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	if (BNXT_PF(bp))
...
{
	return bp->cp_nr_rings +  bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
}
...
static int bnxt_cp_rings_in_use(struct bnxt *bp)
{
	...
	cp += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_CP);
	return cp + bnxt_mpc_cp_rings_in_use(bp);
}

int bnxt_get_func_stat_ctxs(struct bnxt *bp)
{
	return bp->cp_nr_rings + bnxt_get_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX);
}

int bnxt_get_total_rss_ctxs(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	if (!hwr->grp)
		return 0;
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
		...
		if (BNXT_SUPPORTS_NTUPLE_VNIC(bp))
			rss_ctx *= hwr->vnic;
		rss_ctx += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RSS_CTX);
		return rss_ctx;
	}
	if (BNXT_VF(bp))
...
u16 bnxt_get_total_vnics(struct bnxt *bp, int rx_rings)
{
	u16 nr_ulp_vnics = bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC);
	...
	if (bp->flags & BNXT_FLAG_RFS) {
		if (BNXT_SUPPORTS_NTUPLE_VNIC(bp))
			return 2 + bp->num_rss_ctx + nr_ulp_vnics;
		if (!(bp->flags & BNXT_FLAG_CHIP_P5_PLUS))
			return rx_rings + 1;
	}

	return 1 + nr_ulp_vnics;
}

int bnxt_total_rx_rings(struct bnxt *bp)
{
	int rx;
	...
	if (bp->flags & BNXT_FLAG_AGG_RINGS)
		rx <<= 1;
	return rx + bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);
}
...
static void bnxt_get_total_resources(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	...
	if (bp->flags & BNXT_FLAG_AGG_RINGS)
		hwr->rx <<= 1;
	hwr->rx += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);
...
static bool bnxt_trim_ulp_rings(struct bnxt *bp,
				struct bnxt_hw_rings *hwr)
{
	if (hwr->tx < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_TX) ||
	    hwr->rx < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX) ||
	    hwr->cp_p5 < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_CP) ||
	    hwr->vnic < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC) ||
	    hwr->stat < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX) ||
	    hwr->rss_ctx < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RSS_CTX))
		return false;

	hwr->tx -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_TX);
	hwr->rx -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);
	hwr->cp_p5 -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_CP);
	hwr->vnic -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC);
	hwr->stat -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX);
...
static void bnxt_reserve_ulp_rings(struct bnxt *bp,
				   struct bnxt_hw_rings *hwr)
{
	hwr->tx += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_TX);
	hwr->rx += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);
...
static int bnxt_get_avail_ulp_rings(struct bnxt *bp)
{
	int ulp_num_rings_want = bnxt_get_ulp_num_rings(bp);
	int ulp_num_rings_avail = ulp_num_rings_want;
	int max_cp5 = bp->hw_resc.max_cp_rings;
	int max_tx = bp->hw_resc.max_tx_rings;
	int max_rx = bp->hw_resc.max_rx_rings;
	int max_vnics = bp->hw_resc.max_vnics;
	int total_req, nr_tx, nr_rx;
	int nr_vnics, nr_cp5;

	if (!ulp_num_rings_want)
		return 0;

	nr_tx = bp->tx_nr_rings + bnxt_mpc_tx_rings_in_use(bp);
	nr_rx = bp->rx_nr_rings;
	if (bp->flags & BNXT_FLAG_AGG_RINGS)
		nr_rx <<= 1;
	nr_vnics = bp->nr_vnics;
	nr_cp5 = bp->cp_nr_rings;

	if (max_tx == nr_tx || max_rx == nr_rx ||
	    max_vnics == nr_vnics || max_cp5 == nr_cp5)
		return 0;

	total_req = nr_tx + ulp_num_rings_want;
	if (max_tx < total_req) {
		ulp_num_rings_avail = max_tx - nr_tx;
		if (ulp_num_rings_avail <= 0)
			return 0;
	}

	/* ULP requires 2 Rx rings per QP - Rx and Aggr rings */
	total_req = nr_rx + ulp_num_rings_want * 2;
	if (max_rx < total_req) {
		ulp_num_rings_avail = max_rx - nr_rx;
		if (ulp_num_rings_avail <= 0)
			return 0;
	}

	total_req = nr_vnics + ulp_num_rings_want;
	if (max_vnics < total_req) {
		ulp_num_rings_avail = max_vnics - nr_vnics;
		if (ulp_num_rings_avail <= 0)
			return 0;
	}

	/* ULP requires 2 CQ rings per QP - RCQ and SCQ */
	total_req = nr_cp5 + ulp_num_rings_want * 2;
	if (max_cp5 < total_req) {
		ulp_num_rings_avail = max_cp5 - nr_cp5;
		if (ulp_num_rings_avail <= 0)
			return 0;
	}
	return ulp_num_rings_avail;
}
...
{
	int ulp_msix = 0, avail_ulp_rings = 0;
...
retry_resc_resv:
	if (BNXT_NEW_RM(bp) && !bnxt_ulp_registered(bp->edev_rdma)) {
		...
		if (!ulp_msix)
			bnxt_set_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX, 0);
		else
		...
		avail_ulp_rings = bnxt_get_avail_ulp_rings(bp);
		bnxt_set_ulp_num_rings(bp, avail_ulp_rings);
	} else {
		hwr.cp = bnxt_nq_rings_in_use(bp);
	}
	...
	/* When there are not enough resources to support ULP,
	 * reserve again with hwr set to include L2 resources only.
	 */
	if (!bnxt_trim_ulp_rings(bp, &hwr)) {
		bnxt_set_ulp_num_rings(bp, 0);
		memset(&hwr, 0, sizeof(struct bnxt_hw_rings));
		goto retry_resc_resv;
	}
	...
	if (!bnxt_ulp_registered(bp->edev_rdma) && BNXT_NEW_RM(bp)) {
		...
		bnxt_set_ulp_resc(bp, BNXT_ULP_RES_MSIX, ulp_msix);
		resv_ctx = hw_resc->resv_stat_ctxs  - bp->cp_nr_rings;
		ulp_ctxs = min_t(int, resv_ctx, bnxt_get_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX));
		bnxt_set_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX, ulp_ctxs);
	}
...
static int bnxt_get_ctx_inst_cnt(struct bnxt_ctx_mem_type *ctxm)
{
	if (ctxm->instance_bmap)
		return hweight32(ctxm->instance_bmap);
	if (ctxm->max_inst_cnt)
		return ctxm->max_inst_cnt;
	return 1;
}
				   ...
				   struct hwrm_func_backing_store_qcaps_v2_output *resp)
{
	...
	ctxm->max_inst_cnt = le16_to_cpu(resp->max_instance_count);
...
static int bnxt_backing_store_cfg_v2(struct bnxt *bp)
{
	...
	if (BNXT_PF(bp) && (bp->fw_cap & BNXT_FW_CAP_TIMED_TX_PACING)) {
		ctxm = &ctx->ctx_arr[BNXT_CTX_TTXP_TQM];
		if ((ctxm->flags & BNXT_CTX_MEM_TYPE_VALID) &&
		    !bnxt_setup_ctxm_pg_tbls(bp, ctxm, ctxm->max_entries, 0)) {
			last_type = BNXT_CTX_TTXP_TQM;
		} else {
			bnxt_ttxp_free(bp);
			netdev_warn(bp->dev,
				    "Unable to setup Timed TX Pacing.\n");
		}
	}
	if (BNXT_PF(bp)) {
...
int bnxt_hwrm_func_resc_qcaps(struct bnxt *bp, bool all)
{
	...
	hw_resc->max_roce_msix = le16_to_cpu(resp->roce_max_msix);
...
static int __bnxt_hwrm_func_qcaps(struct bnxt *bp)
{
	...
	if (flags_ext2 & FUNC_QCAPS_RESP_FLAGS_EXT2_TIMED_TX_PACING_SUPPORTED)
	...
	if (BNXT_PF(bp) &&
		...
		bp->stats_cap |= BNXT_STATS_CAP_LPBK;
	/* Only support QUIC on 238 FW to prevent backward and forward
	...
	if (flags_ext3 & FUNC_QCAPS_RESP_FLAGS_EXT3_ROCE_CMPL_TS_SUPPORTED)
		bp->fw_cap_ext |= BNXT_FW_CAP_ROCE_CMPL_TS_SUPPORTED;
	if (flags_ext3 & FUNC_QCAPS_RESP_FLAGS_EXT3_VF_ASYM_RESC_ALLOC_SUPPORT)
		bp->fw_cap_ext |= BNXT_FW_CAP_VF_ASYM_RESC_ALLOC_SUPPORT;
	...
	if ((bp->fw_cap & BNXT_FW_CAP_HOT_RESET) && BNXT_CHIP_P5(bp) &&
	    BNXT_SINGLE_PF(bp))
		bp->flags |= BNXT_FLAG_RELOAD_TX_TMO;
	else
		bp->flags &= ~BNXT_FLAG_RELOAD_TX_TMO;
...
static bool bnxt_chip_unsupported(struct bnxt *bp)
{
	return BNXT_CHIP_NUM_588XX(bp->chip_num) ||
...
static int bnxt_hwrm_ver_get(struct bnxt *bp, bool silent)
{
	...
	pkg_len = strnlen(resp->active_pkg_name,
			  sizeof(resp->active_pkg_name));
	if (pkg_len) {
		int fw_ver_len = strlen(bp->fw_ver_str);

		snprintf(bp->fw_ver_str + fw_ver_len,
			 FW_VER_STR_LEN - fw_ver_len - 1,
			 "/pkg %.*s",
			 (int)pkg_len,
	...
	if (bnxt_chip_unsupported(bp)) {
		netdev_err(bp->dev, "Chip ID 0x%x unsupported, aborting\n",
			   bp->chip_num);
		return -ENODEV;
	}
...
skip_rss_ctx:
	...
	rc = bnxt_hwrm_vnic_cfg(bp, vnic, vnic->default_rx_ring);
	if (rc) {
...
{
	return (tx - tx_xdp) / tx_sets + tx_xdp / BNXT_XDP_TX_PER_NAPI;
}
...
static unsigned int bnxt_get_max_func_cp_rings_for_en(struct bnxt *bp)
{
	...
	if (!(bp->flags & BNXT_FLAG_CHIP_P5_PLUS))
		cp -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
...
int bnxt_setup_one_cpr(struct bnxt *bp, bool irq_re_init)
{
	...
	if (rc)
		return rc;
	map_idx = bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
	ring = &cpr->cp_ring_struct;
...
static int bnxt_init_int_mode(struct bnxt *bp)
{
	...
	if (!rc) {
		...
		if (bp->edev_rdma)
			bp->edev_rdma->tph_mode = PCI_TPH_ST_IV_MODE;
	}
#endif
...
int bnxt_reserve_rings(struct bnxt *bp, bool irq_re_init)
{
	...
	if (tcs && (bp->tx_nr_rings_per_tc * tcs !=
		...
		if (bp->tx_nr_rings_xdp)
			bp->tx_nr_rings_per_tc = BNXT_TX_XDP_RINGS_PER_GRP(bp);
		else
				     ...
				     const cpumask_t *mask)
{
	...
	if (!BNXT_SUPPORTS_QUEUE_API(irq->bp))
		return;

	if (!test_bit(BNXT_STATE_OPEN, &irq->bp->state))
		return;
...
static int bnxt_hwrm_phy_qcaps(struct bnxt *bp)
{
	...
	if (!BNXT_CHIP_P5_PLUS(bp) || (le16_to_cpu(resp->flags2) &
	    PORT_PHY_QCAPS_RESP_FLAGS2_PFC_ADV_STATS_SUPPORTED))
		bp->stats_cap |= BNXT_STATS_CAP_PFC_ADV;
...
{
	bp->tx_nr_rings_xdp = bp->tx_nr_rings_per_tc * BNXT_XDP_TX_PER_NAPI;
	bp->tx_nr_rings += bp->tx_nr_rings_xdp;
}
...
#ifdef HAVE_XDP_FRAME
	if (BNXT_TX_XDP_RINGS_PER_GRP(bp) < num_possible_cpus()) {
...
static void bnxt_del_one_uc_filter(struct bnxt *bp, struct bnxt_l2_filter *fltr)
{
	if (!fltr)
		return;
	if (BNXT_PF(bp))
...
static void bnxt_del_all_uc_filters(struct bnxt *bp,
				    struct bnxt_vnic_info *vnic, bool promisc)
{
	int i;

	for (i = 1; i < vnic->uc_filter_count; i++)
		bnxt_del_one_uc_filter(bp, vnic->l2_filters[i]);
	vnic->uc_filter_count = 1;
	if (promisc)
		vnic->rx_mask |= CFA_L2_SET_RX_MASK_REQ_MASK_PROMISCUOUS;
}

static int bnxt_lookup_uc_entry(struct bnxt *bp, struct bnxt_vnic_info *vnic,
				u8 *mac_addr)
{
	int i = 0, off = 0;

	for (; i < vnic->uc_filter_count - 1; i++, off += ETH_ALEN) {
		if (ether_addr_equal(mac_addr, vnic->uc_list + off))
			return i;
	}
	return -ENOENT;
}

static void bnxt_cfg_uc_list(struct bnxt *bp)
{
	struct bnxt_vnic_info *vnic = &bp->vnic_info[BNXT_VNIC_DEFAULT];
	DECLARE_BITMAP(cmn_bmap, BNXT_MAX_UC_ADDRS_PF) = {};
	DECLARE_BITMAP(new_bmap, BNXT_MAX_UC_ADDRS_PF) = {};
	int n, i = 0, j, off = 0, rc, new_uc_count;
	struct bnxt_l2_filter **fltr_tbl;
	struct net_device *dev = bp->dev;
	struct netdev_hw_addr *ha;
	u8 *addr_tbl;

	fltr_tbl = kcalloc(bp->max_uc_fltrs, sizeof(*vnic->l2_filters),
			   GFP_KERNEL);
	if (!fltr_tbl) {
		bnxt_del_all_uc_filters(bp, vnic, true);
		return;
	}
	addr_tbl = kcalloc(bp->max_uc_fltrs, ETH_ALEN, GFP_KERNEL);
	if (!addr_tbl) {
		bnxt_del_all_uc_filters(bp, vnic, true);
		kfree(fltr_tbl);
		return;
	...
	new_uc_count = netdev_uc_count(dev);
	if (new_uc_count > (bp->max_uc_fltrs - 1)) {
		netif_addr_unlock_bh(dev);
		bnxt_del_all_uc_filters(bp, vnic, true);
		goto uc_done;
	}
	netdev_for_each_uc_addr(ha, dev) {
		n = bnxt_lookup_uc_entry(bp, vnic, ha->addr);
		if (n >= 0) {
			__set_bit(n, cmn_bmap);
			fltr_tbl[i + 1] = vnic->l2_filters[n + 1];

		} else {
			__set_bit(i, new_bmap);
		}
		memcpy(addr_tbl + off, ha->addr, ETH_ALEN);
		i++;
		off += ETH_ALEN;
	}
	netif_addr_unlock_bh(dev);

	/* Free the deleted filters in the list */
	for (i = 0; i < vnic->uc_filter_count - 1; i++) {
		if (test_bit(i, cmn_bmap))
			continue;
		bnxt_del_one_uc_filter(bp, vnic->l2_filters[i + 1]);
		vnic->l2_filters[i + 1] = NULL;
	}

	vnic->uc_filter_count = 1;
	for (i = 0, off = 0; i < new_uc_count; i++, off += ETH_ALEN) {
		memcpy(vnic->uc_list + off, addr_tbl + off, ETH_ALEN);
		if (test_bit(i, new_bmap)) {
			/* Add a new filter in the list */
			rc = bnxt_hwrm_set_vnic_filter(bp, 0, i + 1,
						       vnic->uc_list + off);
			if (rc) {
				netdev_err(bp->dev, "HWRM vnic filter failure rc: %d\n", rc);
				bnxt_del_all_uc_filters(bp, vnic, true);
				for (j = i + 1; j < new_uc_count; j++)
					bnxt_del_one_uc_filter(bp, fltr_tbl[j]);
				goto uc_done;
			}
		} else {
			vnic->l2_filters[i + 1] = fltr_tbl[i + 1];
		}
...
static int bnxt_cfg_rx_mode(struct bnxt *bp)
{
	...
	mutex_lock(&bp->ntp_lock);
	bnxt_del_all_uc_filters(bp, vnic, false);

	netif_addr_lock_bh(dev);
	if (netdev_uc_count(dev) > (bp->max_uc_fltrs - 1)) {
		vnic->rx_mask |= CFA_L2_SET_RX_MASK_REQ_MASK_PROMISCUOUS;
	} else {
		netdev_for_each_uc_addr(ha, dev) {
...
#else
		hdr_count++;
#endif /* HAVE_IPV6_BIG_TCP && !HAVE_IPV6_PAYLOAD_LEN */
		nexthdr = &hp->nexthdr;
		start += hdrlen;
	}
	if (nextp) {
...
int bnxt_dbg_hwrm_wr_reg(struct bnxt *bp, u32 reg_off, u32 reg_val)
{
...
static void __bnxt_get_dbg_ring_info_p5(struct bnxt *bp, u32 fw_ring_id,
					u32 fw_cp_ring_id, u8 type, u32 *val)
{
	bnxt_dbg_hwrm_ring_info_get(bp, type, fw_ring_id, &val[0], &val[1]);
	bnxt_dbg_hwrm_ring_info_get(bp, DBG_RING_INFO_GET_REQ_RING_TYPE_L2_CMPL,
				    fw_cp_ring_id, &val[2], &val[3]);
}

void bnxt_get_dbg_tx_ring_info(struct bnxt *bp, struct bnxt_tx_ring_info *txr,
			       u32 *val)
{
	u32 fw_ring_id = txr->tx_ring_struct.fw_ring_id;

	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
		u32 fw_cp_ring_id = txr->tx_cpr->cp_ring_struct.fw_ring_id;

		__bnxt_get_dbg_ring_info_p5(bp, fw_ring_id, fw_cp_ring_id,
					    DBG_RING_INFO_GET_REQ_RING_TYPE_TX,
					    val);
	} else {
		bnxt_dbg_hwrm_rd_reg(bp, BDETBD_REG_BD_PRODUCER_IDX +
				     fw_ring_id * 4, 1, &val[0]);
		bnxt_dbg_hwrm_rd_reg(bp, BDETBD_REG_BD_REQ_CONSUMER_IDX +
				     fw_ring_id * 4, 1, &val[1]);
		bnxt_dbg_hwrm_rd_reg(bp, BDETBD_REG_BD_CMPL_CONSUMER_IDX +
				     fw_ring_id * 4, 1, &val[3]);
	}
}

void bnxt_get_dbg_rx_ring_info(struct bnxt *bp, struct bnxt_rx_ring_info *rxr,
			       u32 fw_ring_id, u32 *val)
{
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
		u32 fw_cp_ring_id = rxr->rx_cpr->cp_ring_struct.fw_ring_id;

		__bnxt_get_dbg_ring_info_p5(bp, fw_ring_id, fw_cp_ring_id,
					    DBG_RING_INFO_GET_REQ_RING_TYPE_RX,
					    val);
	} else {
		bnxt_dbg_hwrm_rd_reg(bp, BDERBD_REG_BD_PRODUCER_IDX +
				     fw_ring_id * 4, 1, &val[0]);
		bnxt_dbg_hwrm_rd_reg(bp, BDERBD_REG_BD_REQ_CONSUMER_IDX +
				     fw_ring_id * 4, 1, &val[1]);
		bnxt_dbg_hwrm_rd_reg(bp, BDERBD_REG_BD_CMPL_CONSUMER_IDX +
				     fw_ring_id * 4, 1, &val[3]);
	}
}

void bnxt_get_dbg_cag_info(struct bnxt *bp, u32 fw_ring_id, u32 *val)
{
	fw_ring_id *= 4;
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_PRODUCER_INDEX_REG + fw_ring_id, 1,
			     val);
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_CONSUMER_INDEX_REG + fw_ring_id, 1,
			     &val[1]);
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_VECTOR_CTRL + fw_ring_id, 1,
			     &val[2]);
}

void bnxt_get_dbg_cag_indir_info(struct bnxt *bp, u32 fw_ring_id, u32 *val)
{
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_PRODUCER_INDEX_REG_ADDR_OFFSET,
			     fw_ring_id);
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_PRODUCER_INDEX_REG, 1, val);
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_PRODUCER_INDEX_REG_ADDR_OFFSET, 0);
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_CONSUMER_INDEX_REG_ADDR_OFFSET,
			     fw_ring_id);
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_CONSUMER_INDEX_REG, 1, &val[1]);
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_CONSUMER_INDEX_REG_ADDR_OFFSET, 0);
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_VECTOR_CTRL_ADDR_OFFSET,
			     fw_ring_id);
	bnxt_dbg_hwrm_rd_reg(bp, CAG_REG_CAG_VECTOR_CTRL, 1, &val[2]);
	bnxt_dbg_hwrm_wr_reg(bp, CAG_REG_CAG_VECTOR_CTRL_ADDR_OFFSET, 0);
}
...
static void bnxt_dbg_dump_hw_ring(struct bnxt *bp, u32 index)
{
	...
	bnxt_for_each_napi_tx(i, bnapi, txr) {
		...
		if (fw_ring_id == INVALID_HW_RING_ID)
			continue;
		bnxt_get_dbg_tx_ring_info(bp, txr, val);
	...
	if (fw_ring_id != INVALID_HW_RING_ID)
		bnxt_get_dbg_rx_ring_info(bp, rxr, fw_ring_id, &val[4]);

	/* AGG RBD prod/cons */
	fw_ring_id = rxr->rx_agg_ring_struct.fw_ring_id;
	if (fw_ring_id != INVALID_HW_RING_ID)
		bnxt_get_dbg_rx_ring_info(bp, rxr, fw_ring_id, &val[8]);

skip_rxr:
	/* CAG prod/cons/vector ctrl */
	fw_ring_id = cpr->cp_ring_struct.fw_ring_id;
	if (fw_ring_id < 1024)
		bnxt_get_dbg_cag_info(bp, fw_ring_id, &val[12]);
	else if (fw_ring_id != INVALID_HW_RING_ID)
		bnxt_get_dbg_cag_indir_info(bp, fw_ring_id, &val[12]);
...
int bnxt_hwrm_monitor_dev_health(struct bnxt *bp, u32 *status)
{
	struct hwrm_monitor_device_health_output *resp;
	struct hwrm_monitor_device_health_input *req;
	int rc;

	rc = hwrm_req_init(bp, req, HWRM_MONITOR_DEVICE_HEALTH);
	if (rc)
		return rc;

	req->type = MONITOR_DEVICE_HEALTH_REQ_TYPE_TX_TIMEOUT;
	resp = hwrm_req_hold(bp, req);
	rc = hwrm_req_send_silent(bp, req);
	if (!rc)
		*status = resp->status;
...
int __bnxt_reset_task(struct bnxt *bp, bool irq_re_init)
{
	int rc;

	pci_dev_lock(bp->pdev);
...
static void bnxt_reset_task(struct bnxt *bp, bool silent)
{
	...
	if (!silent && bp->tx_reporter) {
		int rc;

		rc = devlink_health_report(bp->tx_reporter, "TX error", bp);
		if (!rc)
			return;
	}

	if (!bnxt_hwrm_monitor_dev_health(bp, &status) &&
	    status == MONITOR_DEVICE_HEALTH_RESP_STATUS_FATAL) {
		netdev_err(bp->dev,
			   "FATAL error in chip, FW to initiate error recovery\n");
		return;
	}
...
static void bnxt_tx_timeout(struct net_device *dev, unsigned int txqueue)
{
	...
	if (bp->flags & BNXT_FLAG_CORE_RESET_TX_TIMEOUT) {
		...
		if (bp->flags & BNXT_FLAG_RELOAD_TX_TMO)
			set_bit(BNXT_RELOAD_TASK_SP_EVENT, bp->sp_event);
		else
			set_bit(BNXT_RESET_TASK_SP_EVENT, bp->sp_event);
	}
	__bnxt_queue_sp_work(bp);
...
static void bnxt_timer(unsigned long data)
{
	...
	if (test_bit(BNXT_STATE_FW_ACTIVATE, &bp->state) &&
	    !test_bit(BNXT_STATE_IN_FW_RESET, &bp->state)) {
		if (time_after(jiffies, bp->fw_activate_timestamp + HZ / 10)) {
			struct hwrm_async_event_cmpl cmpl = {};

			bnxt_reset_notify(bp, &cmpl);
			set_bit(BNXT_FW_RESET_NOTIFY_SP_EVENT, bp->sp_event);
			queue_work = true;
			netdev_warn(bp->dev, "Forcing reset event.\n");
		}
	}
...
/* Only called from bnxt_sp_task() */
static void bnxt_reload(struct bnxt *bp)
{
	int rc;
	...
	netdev_info(bp->dev, "Reactivating FW\n");
	/* Use a shorter HWRM timeout to speed up the shutdown */
	bp->hwrm_cmd_timeout = DFLT_HWRM_CMD_TIMEOUT;
	set_bit(BNXT_STATE_FW_ACTIVATE, &bp->state);
	bp->fw_activate_timestamp = jiffies;
	...
	if (rc) {
		...
		bnxt_rtnl_unlock_sp(bp);
		netdev_warn(bp->dev, "Failed to activate firmware\n");
		return;
	}
	bnxt_rtnl_unlock_sp(bp);
	netdev_info(bp->dev, "FW activate should complete within %d msecs\n",
		    bp->fw_reset_max_dsecs * 100);
}
...
static void bnxt_init_ethtool_link_settings(struct bnxt *bp)
{
	...
	if (BNXT_AUTO_MODE(link_info->auto_mode)) {
		...
		link_info->autoneg = BNXT_AUTONEG_NONE;
	}
	if (link_info->autoneg & BNXT_AUTONEG_FLOW_CTRL)
		link_info->req_flow_ctrl =
			link_info->auto_pause_setting & BNXT_LINK_PAUSE_BOTH;
...
static void bnxt_sp_task(struct work_struct *work)
{
	...
	if ((!netif_running(bp->dev) || !test_bit(BNXT_STATE_OPEN, &bp->state)) &&
	...
	/* Handle VF FLR even when the PF is not open */
	if (test_and_clear_bit(BNXT_VF_FLR_SP_EVENT, bp->sp_event)) {
		u16 fid;

		/* Handle multiple FLRs which may be queued up */
		fid = find_first_bit(bp->tbl_scope_vf_ids, BNXT_MAX_TBL_SCOPE_VFS);
		while (fid  < BNXT_MAX_TBL_SCOPE_VFS) {
			clear_bit(fid, bp->tbl_scope_vf_ids);
			if (bp->tfp)
				tfc_tbl_scope_func_reset(bp->tfp, fid + bp->pf.first_vf_id);
			fid = find_first_bit(bp->tbl_scope_vf_ids, BNXT_MAX_TBL_SCOPE_VFS);
		}
	}
	...
	if (test_and_clear_bit(BNXT_RELOAD_TASK_SP_EVENT, bp->sp_event))
		bnxt_reload(bp);
...
ifdown_reset:
	if (test_and_clear_bit(BNXT_FW_RESET_NOTIFY_SP_EVENT, bp->sp_event)) {
		/* Synchronize with user/sysfs initiated FLR */
		pci_dev_lock(bp->pdev);
		if ((test_bit(BNXT_STATE_FW_FATAL_COND, &bp->state) ||
		     ...
		     int tx_xdp)
{
	...
	if (!rc && pci_msix_can_alloc_dyn(bp->pdev)) {
		if (!bnxt_ulp_registered(bp->edev_rdma)) {
			hwr.cp += bnxt_get_ulp_resc(bp, BNXT_ULP_RES_MSIX);
...
static int bnxt_fw_init_one_p1(struct bnxt *bp)
{
	...
	if (rc == -ENODEV && bnxt_chip_unsupported(bp))
		return -ENODEV;
...
static int bnxt_setup_etf_tc(struct net_device *dev, struct tc_etf_qopt_offload *qopt)
{
	...
	if (ttxp && ttxp->ring_rates[qopt->queue]) {
		netdev_warn(dev, "Tx packet queue %d already has pacing enabled.\n",
			    qopt->queue);
		return -EOPNOTSUPP;
	}
...
static const struct udp_tunnel_nic_info bnxt_udp_tunnels = {
	...
	.flags		= BNXT_UDP_TUNNEL_NIC_INFO_OPEN_ONLY,
	.tables		= {
...
#endif
	.ndo_set_tx_maxrate = bnxt_set_tx_maxrate,
...
static int bnxt_tx_queue_start(struct bnxt *bp, int idx)
{
	...
	bnxt_for_each_napi_tx(i, bnapi, txr) {
		...
		tx_idx = txr->txq_index + bp->tx_nr_rings_xdp;
...
void _bnxt_get_max_rings(struct bnxt *bp, int *max_rx, int *max_tx,
			 int *max_cp)
{
	struct bnxt_hw_resc *hw_resc = &bp->hw_resc;
	...
	*max_tx -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_TX);
	*max_rx = hw_resc->max_rx_rings;
	*max_rx -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);
...
static int bnxt_set_dflt_rings(struct bnxt *bp, bool sh)
{
	...
	dflt_rings = bnxt_compat_adjust_dflt_rss_queues(dflt_rings);
	...
	/* Ask for 16 rings per partition, so RoCE can loaded */
	if (BNXT_NPAR(bp))
		dflt_rings = min_t(int, dflt_rings, BNXT_NUM_DFLT_RINGS_NPAR);
	...
	if (avail_msix >= BNXT_MIN_ROCE_CP_RINGS) {
		...
		bnxt_set_ulp_resc(bp, BNXT_ULP_RES_MSIX, ulp_num_msix);
		bnxt_set_dflt_ulp_stat_ctxs(bp);
	}
...
static bool bnxt_is_member_port(struct bnxt *bp, struct net_device *netdev)
{
	...
	peer_dsn = pci_get_dsn(event_pdev);
	dsn = get_unaligned_le64(bp->dsn);
...
static void bnxt_hwrm_lag_update(struct bnxt *bp)
{
	...
	/* Only PF0 may issue LAG update/create commands */
	if (bp->pf.port_id)
		return;
...
static void bnxt_hndl_mtu_change(struct bnxt *bp, void *ptr)
{
...
static void bnxt_hndl_ndev_change(struct bnxt *bp, void *ptr)
{
	...
	if (!bnxt_is_member_port(bp, netdev))
		return;
	port_id = ((struct bnxt *)netdev_priv(netdev))->pf.port_id;
	if (!netif_is_lag_port(bp->dev) || !netif_is_lag_port(netdev))
		return;
	if (!test_bit(port_id, &binfo->member_port_map))
		return;
	a_port_map = 0;
	...
	bond_for_each_slave(bond, iter_slave, iter) {
		...
		tmp_port_id = tmp_bp->pf.port_id;
		if (!bond_slave_can_tx(iter_slave))
			__clear_bit(tmp_port_id, &a_port_map);
		else
			__set_bit(tmp_port_id, &a_port_map);
	}
	if (bnxt_hwrm_get_fw_lag_id(bp))
		return;
	if (a_port_map != binfo->active_port_map ||
	    (binfo->fw_lag_id == BNXT_INVALID_LAG_ID &&
	     hweight_long(binfo->member_port_map) == BNXT_PORTS_MAX)) {
...
static void bnxt_sync_peers_from_bond(struct net_device *bond, struct bnxt *bp)
{
	struct bnxt_bond_info *binfo = bp->bond_info;
	...
	netdev_for_each_lower_dev(bond, netdev, iter) {
		if (!bnxt_is_member_port(bp, netdev))
			continue;
		port_id = (((struct bnxt *)netdev_priv(netdev))->pf.port_id);
		if (!__test_and_set_bit(port_id, &bp->bond_info->peers))
			bp->bond_info->p_netdev[port_id] = netdev;
	}
}
...
static void bnxt_hwrm_lag_free(struct bnxt *bp)
{
	...
	/* Only PF0 may issue LAG free command */
	if (bp->pf.port_id)
		return;
...
static void bnxt_teardown_lag(struct bnxt *bp)
{
	struct bnxt_bond_info *binfo = bp->bond_info;
...
static bool bnxt_bond_is_valid_for_lag(struct bnxt *bp, struct net_device *bond_master)
{
	struct net_device *lower_dev;
	struct list_head *iter;
	int bnxt_members = 0;

	netdev_for_each_lower_dev(bond_master, lower_dev, iter) {
		if (!bnxt_is_member_port(bp, lower_dev))
			return false;
		bnxt_members++;
	}
	return bnxt_members == BNXT_PORTS_MAX;
}
...
static void bnxt_hndl_changeupper(struct bnxt *bp, void *ptr)
{
	...
	if (!bnxt_is_member_port(bp, netdev) &&
	    bnxt_is_netdev_bond_slave(dev, bp)) {
		if (!info->linking && mode &&
		    bnxt_bond_is_valid_for_lag(bp, dev)) {
			bnxt_sync_peers_from_bond(dev, bp);
			binfo->member_port_map = binfo->peers;
			binfo->aggr_mode = mode;
			binfo->bond_active = true;
			bnxt_hwrm_lag_update(bp);
			return;
		}
		bnxt_teardown_lag(bp);
		return;
	}
	...
	if (info->linking) {
		...
		bnxt_sync_peers_from_bond(dev, bp);
		binfo->member_port_map = binfo->peers;
		if (!bnxt_bond_is_valid_for_lag(bp, dev)) {
			bnxt_teardown_lag(bp);
			if (own_event)
				bnxt_tf_port_reinit(bp);
			return;
		}
		/* Initialize LAG state on first creation;
		 * send lag_update for both own and peer events
		 */
		if (binfo->fw_lag_id == BNXT_INVALID_LAG_ID) {
			binfo->aggr_mode = mode;
			binfo->bond_active = true;
...
static void bnxt_sync_bond_info(struct bnxt *bp, void *ptr)
{
	...
	if (!bnxt_is_member_port(bp, netdev))
		return;
	if (netdev != bp->dev || binfo->fw_lag_id != BNXT_INVALID_LAG_ID)
		return;
	if (bnxt_hwrm_get_fw_lag_id(bp))
		return;
	if (binfo->fw_lag_id == BNXT_INVALID_LAG_ID)
	...
	if (__test_and_clear_bit(bp->pf.port_id, &binfo->member_port_map)) {
		if (hweight_long(binfo->member_port_map) < 2)
			bnxt_teardown_lag(bp);
	}
}
...
static int bnxt_enable_atomic_ops_to_root(struct bnxt *bp)
{
	/* Do not enable on P4/P8 chips and on VF */
	if (!(BNXT_CHIP_P5(bp) || BNXT_CHIP_P7(bp)) || BNXT_VF(bp))
		return 0;

	if (pci_enable_atomic_ops_to_root(bp->pdev,
...
static int bnxt_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (!(bp->tunnel_disable_flag & FUNC_QCAPS_RESP_TUNNEL_DISABLE_FLAG_DISABLE_IPINIP)) {
		dev->hw_features |= NETIF_F_GSO_IPXIP4 | NETIF_F_GSO_IPXIP6;
#ifdef NETDEV_HW_ENC_FEATURES
		dev->hw_enc_features |= NETIF_F_GSO_IPXIP4 | NETIF_F_GSO_IPXIP6;
#endif
	}
	...
	if (BNXT_PF(bp)) {
		...
		bp->max_uc_fltrs = BNXT_MAX_UC_ADDRS_PF;
	} else {
		bp->max_uc_fltrs = BNXT_MAX_UC_ADDRS_VF;
	}
	...
	if (bp->flags & BNXT_FLAG_ROCE_CAP) {
		rc = bnxt_enable_atomic_ops_to_root(bp);
		if (rc)
			netdev_dbg(bp->dev, "Enabling pci atomics failed\n");
	}
...
static int bnxt_pci_reset_prepare_common(struct pci_dev *pdev)
{
...
static void bnxt_pci_reset_teardown(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct bnxt *bp = netdev_priv(netdev);
...
static int bnxt_pci_reset_reinit(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct bnxt *bp = netdev_priv(netdev);
	int retry = 0;
	...
	if (err) {
		dev_err(&pdev->dev, "HWRM func reset failed\n");
		return err;
	}
					       ...
					       pci_channel_state_t state)
{
	...
	netdev_lock(netdev);
	if (bnxt_pci_reset_prepare_common(pdev) ||
...
static pci_ers_result_t bnxt_io_slot_reset(struct pci_dev *pdev)
{
	...
	err = bnxt_pci_reset_reinit(pdev);
	if (!err)
...
/**
 * bnxt_pci_reset_prepare - Prepare device for PCIe FLR
 * @pdev: Pointer to PCI device
 */
static void bnxt_pci_reset_prepare(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct bnxt *bp = netdev_priv(netdev);

	netdev_info(netdev, "PCI reset prepare (FLR)\n");

	if (pci_num_vf(bp->pdev))
		netdev_warn(netdev, "Active VFs present during PCIe FLR; VFs will not be functional.\n");
...
/**
 * bnxt_pci_reset_done - Re-initialize device after PCIe FLR
 * @pdev: Pointer to PCI device
 */
static void bnxt_pci_reset_done(struct pci_dev *pdev)
{
	struct net_device *netdev = pci_get_drvdata(pdev);
	struct bnxt *bp = netdev_priv(netdev);
	int err;

	netdev_info(bp->dev, "PCI reset done (FLR)\n");
	netdev_lock(netdev);

	err = bnxt_pci_reset_reinit(pdev);
	if (err)
		dev_err(&pdev->dev, "FLR failed, re-load the driver to recover\n");
...
static const struct pci_error_handlers bnxt_err_handler = {
	...
	.reset_prepare  = bnxt_pci_reset_prepare,
	.reset_done     = bnxt_pci_reset_done,
...
static struct pci_driver bnxt_pci_driver = {
	...
	.sriov_configure = bnxt_sriov_configure,
#if defined(HAVE_SRIOV_SET_MSIX)
	.sriov_get_vf_total_msix = bnxt_sriov_get_vf_total_msix,
	.sriov_set_msix_vec_count = bnxt_sriov_set_msix_vec_count,
#endif
#else
