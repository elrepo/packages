...
struct tx_bd_ext {
	...
	#define TX_BD_FLAGS_ROCE_CRC				(1 << 8)
...
struct tx_bd_timed {
	...
	#define TX_BD_FLAGS_KIND_ASAP				(0x0UL << 6)
	#define TX_BD_FLAGS_KIND_SO_TXTIME			(0x1UL << 6)
	#define TX_BD_FLAGS_KIND_PACE				(0x2UL << 6)
	__le32 rate;
	#define TX_BD_TIMED_RATE_MASK				0x1ffffff
	__le64 tx_time;
};
...
struct bnxt_coal {
	...
	u8			budget;
	u16			flags;
	u16			ring_alloc_flags;
};

#define BNXT_TMR_RST_SUPPORTED(bp)				\
	((bp)->coal_cap.cmpl_params &				\
	 RING_AGGINT_QCAPS_RESP_CMPL_PARAMS_TIMER_RESET)

#define BNXT_TMR_RST_ON_RING_ALLOC_SUPPORTED(bp)		\
	((bp)->coal_cap.cmpl_params &				\
	 RING_AGGINT_QCAPS_RESP_CMPL_PARAMS_TMR_RESET_ON_ALLOC)

#define BNXT_COAL_GET_CQE(bp, coal)				\
	(BNXT_TMR_RST_SUPPORTED(bp) ?				\
		((coal)->flags & RING_CMPL_RING_CFG_AGGINT_PARAMS_REQ_FLAGS_TIMER_RESET) : \
		(BNXT_TMR_RST_ON_RING_ALLOC_SUPPORTED(bp) ?	\
			((coal)->ring_alloc_flags & RING_ALLOC_REQ_FLAGS_TIMER_RESET) : 0))

#define BNXT_COAL_SET_CQE(bp, coal)				\
do {								\
	if (BNXT_TMR_RST_SUPPORTED(bp))				\
		(coal)->flags |=				\
			RING_CMPL_RING_CFG_AGGINT_PARAMS_REQ_FLAGS_TIMER_RESET; \
	else if (BNXT_TMR_RST_ON_RING_ALLOC_SUPPORTED(bp))	\
		(coal)->ring_alloc_flags |=			\
			RING_ALLOC_REQ_FLAGS_TIMER_RESET;	\
} while (0)

#define BNXT_COAL_CLEAR_CQE(bp, coal)				\
do {								\
	(coal)->flags &= ~RING_CMPL_RING_CFG_AGGINT_PARAMS_REQ_FLAGS_TIMER_RESET; \
	(coal)->ring_alloc_flags &= ~RING_ALLOC_REQ_FLAGS_TIMER_RESET; \
} while (0)
...
#define BNXT_MAX_CTX_PER_VNIC	8
	...
	u16		fw_l2_ctx_id;
	u16		mru;
#define BNXT_MAX_UC_ADDRS_PF	128
#define BNXT_MAX_UC_ADDRS_VF	4
	struct bnxt_l2_filter **l2_filters;
...
#define NUM_BNXT_CRYPTO_TYPES (BNXT_CRYPTO_TYPE_QUIC + 1)
...
struct bnxt_hw_resc {
	...
	struct bnxt_hw_tls_resc tls_resc[NUM_BNXT_CRYPTO_TYPES];
};
...
struct bnxt_filter_base {
	...
	union {
		__le64		l2_filter_id;
		__le64		ntp_filter_id[BNXT_MAX_NTP_REP];
...
#define BNXT_NUM_DFLT_RINGS_NPAR	16
...
#define BNXT_CTX_TTXP_TQM		FUNC_BACKING_STORE_QCAPS_V2_REQ_TYPE_TTX_PACING_TQM_RING
...
#define BNXT_VPD_FLD_LEN	32
	...
	#define BNXT_FLAG_RELOAD_TX_TMO	0x40
...
#define BNXT_CHIP_SUPPORTS_PHY(bp)	(BNXT_ASIC(bp) || BNXT_CHIP_P7_PLUS(bp))
	...
	int			tx_nr_rings_xdp; /* total for TX/REDIRECT */
...
#define BNXT_STATE_RESET_NOTIFY_BEGIN	20
...
#define BNXT_NO_FW_ACCESS(bp)					\
	...
	#define BNXT_FW_CAP_TIMED_TX_PACING		BIT_ULL(44)
	...
	#define BNXT_FW_CAP_ROCE_CMPL_TS_SUPPORTED	BIT_ULL(6)
	#define BNXT_ROCE_CMPL_TS_SUPPORTED(bp) ((bp)->fw_cap_ext & \
						 BNXT_FW_CAP_ROCE_CMPL_TS_SUPPORTED)
	...
	#define BNXT_FW_CAP_VF_ASYM_RESC_ALLOC_SUPPORT	BIT_ULL(10)
	...
	#define BNXT_STATS_CAP_PFC_ADV		BIT(12)
...
#define BNXT_PTP_USE_RTC(bp)	(!BNXT_MH(bp) && \
	...
	void			*hwrm_polling_req;
	void			*hwrm_polling_resp;
...
#define BNXT_DFLT_FW_RST_MAX_DSECS	60
	...
	unsigned long		fw_activate_timestamp;
...
#define BNXT_GET_EXT_RING_STATS64(sw, counter)		\
	(*((sw) + offsetof(struct ctx_hw_stats_ext, counter) / 8))
