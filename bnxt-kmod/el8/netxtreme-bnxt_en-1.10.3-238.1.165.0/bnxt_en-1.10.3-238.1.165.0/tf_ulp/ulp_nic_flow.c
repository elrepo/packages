...
/*
 * "CMA probe matching in CFA"
 *
 * The best is match on
 * bth.qpn=1 with l4.acknum
 * bth.opcode=100  with l4.dest
 * bth.ack=1 with l4.flags[4]
 * bth.padcnt=0 with l4.flag[1:0],
 * padcnt= default is 0,
 * or sysfs echoed value.it's configurable through sysfs
 */

static int roce_probe_flow_create(struct bnxt *bp,
				  u32 *cnp_flow_id, u64 *cnp_flow_cnt_hndl,
				  bool is_v6, enum cfa_dir dir,
				  u16 pad_cnt, u8 ack, u8 ack_m)
{
	struct bnxt_ulp_gen_bth_hdr bth_spec = { 0 }, bth_mask = { 0 };
	struct bnxt_ulp_gen_ipv6_hdr v6_spec = { 0 }, v6_mask = { 0 };
	struct bnxt_ulp_gen_ipv4_hdr v4_spec = { 0 }, v4_mask = { 0 };
	...
	u32 dst_qpn = cpu_to_be32(0x1);
	u16 opcode_mask = cpu_to_be16(0xff);
	u16 opcode = cpu_to_be16(100);
	u16 flags_mask = cpu_to_be16(0x3 | (ack_m << 4));
	u16 flags = cpu_to_be16(pad_cnt | (ack << 4));
	u8 l4_proto = IPPROTO_UDP;
	u8 l4_proto_mask = 0xff;
	int rc = 0;
	...
	bth_spec.op_code = &opcode;
	bth_mask.op_code = &opcode_mask;
				...
				enum cfa_dir dir)
{
	...
	if (dir == CFA_DIR_RX) {
		...
		/*
		 * Don't install probe flow if probe pad count is 0,
		 * unless ACK/RSVD7 capability is advertized
		 */
		if (!rc && (bp->udcc_info->pad_cnt || BNXT_UDCC_BTH_ACK_RSVD7_EN(bp))) {
			netdev_dbg(bp->dev, "UDCC probe config PAD:%x ACK:%x/%x\n",
				   bp->udcc_info->pad_cnt,
				   bp->udcc_info->probe_bth_ack,
				   bp->udcc_info->probe_bth_ack_m);
