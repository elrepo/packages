...
/* physmem alloc list for mmap/IOCFREE validation */
struct bnxt_lfc_phys_alloc {
...
static LIST_HEAD(bnxt_lfc_phys_alloc_list);
			    ...
			    unsigned long event, void *ptr)
{
	...
	if (pdev->vendor != PCI_VENDOR_ID_BROADCOM ||
	    strcmp(dev_driver_string(&pdev->dev), "bnxt_en"))
		return NOTIFY_DONE;

	hash = bnxt_lfc_get_hash_key(pdev->bus->number, pdev->devfn);

	mutex_lock(&blfc_global_dev.bnxt_lfc_lock);
	if (event == NETDEV_REGISTER) {
		blfc_array[hash].removed = 0;
	} else {
		blfc_dev = blfc_array[hash].bnxt_lfc_dev;
		if (blfc_dev && blfc_dev->ndev == dev) {
			dev_put(blfc_dev->ndev);
			kfree(blfc_dev);
			blfc_array[hash].bnxt_lfc_dev = NULL;
			blfc_array[hash].taken = 0;
			blfc_array[hash].removed = 1;
		} else if (!blfc_array[hash].taken) {
			/* Slot was never cached (no prior HWRM ioctl for this
			 * BDF).  Mark it removed so a concurrent or future
			 * ioctl does not race with hot-unplug teardown.
			 */
			blfc_array[hash].removed = 1;
		}
		/* else: slot is taken but ndev does not match — hash
		 * collision from a different device (e.g. a VF whose
		 * bus+devfn hashes to the same slot as a PF).  Leave
		 * the PF's removed flag untouched.
		 */
	}
					       ...
					       struct bnxt_fw_msg *fw_msg)
{
	...
	for (i = 0; i < msg->num_dma_indications; i++) {
		...
		if ((PTR_ALIGN(dma_ptr, 8) == dma_ptr) &&
		    (msg->len_req >= sizeof(__le64) &&
		     msg->dma[i].offset <= msg->len_req - sizeof(__le64))) {
			__le64 *dmap = dma_ptr;
			...
			struct bnxt_lfc_req *lfc_req)
{
	...
	if (msg.num_dma_indications) {
		...
		/* Pin validated header fields to prevent TOCTOU via the second copy_from_user. */
		msg2->num_dma_indications = msg.num_dma_indications;
		msg2->len_req = msg.len_req;
		msg2->len_resp = msg.len_resp;
...
{
	struct bnxt_lfc_phys_alloc *alloc_entry;
	struct pci_dev *pdev = blfc_dev->pdev;
	...
	/* Allocate tracking entry before DMA alloc so we can fail cleanly */
	alloc_entry = kmalloc(sizeof(*alloc_entry), GFP_KERNEL);
	if (!alloc_entry)
		return -ENOMEM;
	...
	/* Record allocation so mmap and free can validate it */
	alloc_entry->phys_addr = addr;
	alloc_entry->bus_addr = bus_addr;
	alloc_entry->size = data.size;
	list_add(&alloc_entry->list, &bnxt_lfc_phys_alloc_list);
...
{
	struct bnxt_lfc_phys_alloc *entry, *tmp;
	struct pci_dev *pdev = blfc_dev->pdev;
	...
	/* Reject any address not previously allocated by this driver.
	 * The mutex is already held by bnxt_lfc_ioctl().
	 */
	list_for_each_entry_safe(entry, tmp, &bnxt_lfc_phys_alloc_list, list) {
		if (entry->phys_addr == data.phys_addr &&
		    entry->bus_addr == data.bus_addr &&
	...
	if (!found) {
		BNXT_LFC_ERR(&blfc_dev->pdev->dev,
			     "Invalid free: address not allocated by driver\n");
		return -EINVAL;
	}
...
static int bnxt_lfc_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	unsigned long req_pfn = vma->vm_pgoff;
	struct bnxt_lfc_phys_alloc *entry;
	bool found = false;

	/* Permit mapping only of memory previously allocated via
	 * BNXT_LFC_IOCALLOC_PHYS_MEM; reject every other PFN.
	 */
	mutex_lock(&blfc_global_dev.bnxt_lfc_lock);
	list_for_each_entry(entry, &bnxt_lfc_phys_alloc_list, list) {
		unsigned long alloc_pfn = entry->phys_addr >> PAGE_SHIFT;
		unsigned long alloc_pages = PFN_UP(entry->size);

		if (req_pfn >= alloc_pfn &&
		    req_pfn + (size >> PAGE_SHIFT) <= alloc_pfn + alloc_pages) {
			found = true;
			break;
		}
	}
