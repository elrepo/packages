			     ...
			     u16 *resp_len)
{
	...
	if (req->dir < 0 || req->dir >= CFA_DIR_MAX) {
		netdev_dbg(bp->dev, "%s: invalid dir %d\n", __func__, req->dir);
		return -EINVAL;
	}

	if (req->region < 0 || req->region >= CFA_REGION_TYPE_MAX) {
		netdev_dbg(bp->dev, "%s: invalid region %d\n", __func__, req->region);
		return -EINVAL;
	}
			    ...
			    u16 *resp_len)
{
	...
	*resp_len = cpu_to_le16(data_len);
	resp->hdr.type = TFC_VF2PF_TYPE_TBL_SCOPE_POOL_FREE_CMD;
	resp->tsid = req->tsid;
	resp->status = rc;
	return rc;
}
