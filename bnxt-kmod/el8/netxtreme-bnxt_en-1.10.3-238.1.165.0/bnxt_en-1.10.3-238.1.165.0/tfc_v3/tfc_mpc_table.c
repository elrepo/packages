				...
				int level)
{
	...
	/* compress until end of file; bounded chunks avoid u32 underflow */
	do {
		u32 n = min_t(u32, chunk_size, in_size);

		strm.avail_in = n;
		source = (u8 *)get_address(mem_cfg, offset);
		if (!source) {
			netdev_warn(bp->dev,
				    "%s: no page at offset %u\n",
				    __func__, offset);
			ret = -EFAULT;
			goto err;
		}
		...
		strm.next_in = in;
		offset += n;
		flush = (n == in_size ? Z_FINISH : Z_NO_FLUSH);
		in_size -= n;
