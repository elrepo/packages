...
struct tfc_global_object {
	...
	u16 vf_cnt;					/* total vf count on all PFs */
};
...
static struct tfc_global_object *tfc_global_obj_init(struct bnxt *bp)
{
	...
	tfgo->client_cnt++;
	if (bp->pf.max_vfs)
		tfgo->vf_cnt += bp->pf.max_vfs;

	netdev_dbg(bp->dev, "Global object clients(%d) and vf_cnt(%d)\n",
		   tfgo->client_cnt, tfgo->vf_cnt);
...
static void tfc_global_obj_deinit(struct bnxt *bp)
{
	...
	if (tfgo->client_cnt) {
		...
		tfgo->client_cnt--;
		/* no need to decrement vf_cnt, reset implicit via struct free */
	}

	if (tfgo->client_cnt == 0) {
		netdev_dbg(bp->dev, "%s: last client removed\n", __func__);
		/* Run VF table-scope cleanup here so it runs exactly when we become
		 * the last PF (after decrement). Avoids concurrent teardown race
		 * where cleanup_if_last() saw client_cnt > 1 for both PFs.
		 */
		if (bp->tfp)
...
int tfo_get_max_fid(void *tfo, u16 *max_fid)
{
	struct tfc_object *tfco;
	struct tfc_global_object *tfgo;
	...
	if (tfgo->vf_cnt)
		*max_fid = BNXT_FIRST_VF_FID + tfgo->vf_cnt;
	else
		*max_fid = BNXT_FIRST_PF_FID + tfgo->client_cnt;
