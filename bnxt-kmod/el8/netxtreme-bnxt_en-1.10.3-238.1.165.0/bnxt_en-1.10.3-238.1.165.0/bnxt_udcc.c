...
int bnxt_hwrm_udcc_cfg(struct bnxt *bp, u32 enables, u8 mode, u8 padcnt, u8 ack, u8 rsvd7)
{
	...
	if (enables & UDCC_CFG_REQ_ENABLES_PROBE_BTH_ACK)
		req->probe_bth_ack = ack;

	if (enables & UDCC_CFG_REQ_ENABLES_PROBE_RESP_BTH_RSVD7)
		req->probe_resp_bth_rsvd7 = rsvd7;
...
int bnxt_hwrm_udcc_qcfg(struct bnxt *bp)
{
	...
	if (BNXT_UDCC_BTH_ACK_RSVD7_EN(bp)) {
		bp->udcc_info->probe_bth_ack = resp->probe_bth_ack;
		bp->udcc_info->probe_bth_ack_m = 1;
	} else {
		bp->udcc_info->probe_bth_ack = 0;
		bp->udcc_info->probe_bth_ack_m = 0;
	}
...
{
	u8 probe_pad_cnt, probe_bth_ack = 0, probe_resp_bth_rsvd7 = 0;
	struct bnxt_udcc_info *udcc = bp->udcc_info;
	struct hwrm_udcc_qcaps_output *resp;
	struct hwrm_func_qcaps_input *req;
	u32 udcc_probe_enables;
	int rc;
	...
	/* Configure default probe pad count value based on ACK/RSVD7 capability flag */
	if (BNXT_UDCC_BTH_ACK_RSVD7_EN(bp)) {
		udcc_probe_enables = UDCC_CFG_REQ_ENABLES_PROBE_PAD_CNT_CFG |
				     UDCC_CFG_REQ_ENABLES_PROBE_BTH_ACK |
				     UDCC_CFG_REQ_ENABLES_PROBE_RESP_BTH_RSVD7;
		probe_pad_cnt = UDCC_CFG_REQ_PROBE_PAD_CNT_CFG_DISABLED;
		probe_bth_ack = 1;
		probe_resp_bth_rsvd7 = 0x40;
	} else {
		udcc_probe_enables = UDCC_CFG_REQ_ENABLES_PROBE_PAD_CNT_CFG;
		probe_pad_cnt = UDCC_CFG_REQ_PROBE_PAD_CNT_CFG_THREE;
	}
	...
	if (BNXT_UDCC_DCQCN_EN(bp))
		...
		netdev_info(bp->dev, "%s UDCC enabled!!! in %s (max %d sessions). %s\n",
			    udcc->session_type ? "Per-QP" : "Per-DestIP",
			    udcc->hybrid_mode ? "Hybrid mode" :
			    "Non-Hybrid mode",
			    udcc->max_sessions,
			    BNXT_UDCC_BTH_ACK_RSVD7_EN(bp) ?
			    "CMA Probe BTH.ack match capable" : "");
	else
