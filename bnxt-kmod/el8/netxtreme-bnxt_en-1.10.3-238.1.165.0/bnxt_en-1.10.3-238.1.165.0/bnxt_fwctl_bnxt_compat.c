...
#ifndef FWCTL_DEVICE_TYPE_BNXT
#define FWCTL_DEVICE_TYPE_BNXT 3
#endif
...
static int bnxt_fw_setup_input_dma(struct device *dev,
				   int num_dma,
				   ...
				   dma_addr_t dma_addr[])
{
	...
	for (i = 0; i < num_dma; i++) {
		...
		dma_virt_addr[i] = dma_alloc_coherent(dev->parent,
						      msg->len,
						      &dma_addr[i],
						      GFP_KERNEL);
		if (!dma_virt_addr[i]) {
		...
		if ((PTR_ALIGN(dma_ptr, 8) == dma_ptr) &&
		    (fw_msg->msg_len >= sizeof(__le64) &&
		     msg->offset <= fw_msg->msg_len - sizeof(__le64))) {
			__le64 *dmap = dma_ptr;
			    ...
			    void *in, size_t in_len, size_t *out_len)
{
	...
	if ((in_len < sizeof(struct fwctl_rpc_bnxt)) ||
	    (*out_len < sizeof(struct output)))
	...
	if (msg->num_dma) {
		...
		rc = bnxt_fw_setup_input_dma(dev, msg->num_dma,
					     dma_buf, &rpc_in,
					     dma_virt_addr, dma_addr);
		if (rc) {
			err = -EOPNOTSUPP;
			goto err_out;
		}
...
err_out:
	if (msg->num_dma && dma_buf)  {
		for (i = 0; i < msg->num_dma && dma_virt_addr[i]; i++)
			 ...
			 const struct auxiliary_device_id *id)
{
	...
	bnxtctl = fwctl_alloc_device(&aux_priv->edev->pdev->dev, &bnxtctl_ops,
	...
	if (rc) {
		fwctl_put(&bnxtctl->fwctl);
		return rc;
	}
	auxiliary_set_drvdata(adev, bnxtctl);
	return 0;
}
