 ...
 * this PF or bond of this PF, by comparing with the lower_dev of vxlan-dev.
 * BOND can be child of VxLAN, then make sure this BOND's PF is our PF
 */
static bool bnxt_is_vxlan_lower_dev(struct net_device *vxlan_netdev,
				    struct bnxt *bp)
{
	const struct vxlan_dev *vxlan = NULL;
	const struct vxlan_rdst *dst = NULL;
	const struct net_device *remote_dev = NULL;
	const struct net_device *master_dev = NULL;

	if (!netif_is_vxlan(vxlan_netdev))
		return false;

	vxlan = netdev_priv(vxlan_netdev);
	dst = &vxlan->default_dst;

#ifdef HAVE_VXLAN_RDST_RDEV
	remote_dev = dst->remote_dev;
#else
	if (dst->remote_ifindex)
		remote_dev = __dev_get_by_index(dev_net(bp->dev),
						dst->remote_ifindex);
#endif

	if (remote_dev) {
		if (netif_is_lag_master(remote_dev)) {
			rcu_read_lock();
			master_dev = netdev_master_upper_dev_get_rcu(bp->dev);
			rcu_read_unlock();

			if (master_dev == remote_dev)
				return true;
			else
				return false;
		} else if (bp->dev == remote_dev) {
			return true;
		} else {
			return false;
		}
...
/* If the netdev is bond, then ensure that this bond is for our bp's dev */
static bool bnxt_is_bond_lower_dev(struct net_device *netdev,
				   struct bnxt *bp)
{
	const struct net_device *bond_dev = NULL;

	if (netif_is_lag_master(netdev)) {
		rcu_read_lock();
		bond_dev = netdev_master_upper_dev_get_rcu(bp->dev);
		rcu_read_unlock();
		if (bond_dev == netdev)
			return true;
	}
...
#endif
{
	...
	case FLOW_BLOCK_BIND:
		if (!(bnxt_is_bond_lower_dev(netdev, bp) ||
		      bnxt_is_vxlan_lower_dev(netdev, bp))) {
			return -EOPNOTSUPP;
		}
		...
		netdev_dbg(bp->dev, "FLOW BLOCK BIND %s\n", netdev->name);
	...
	case FLOW_BLOCK_UNBIND:
		...
		netdev_dbg(bp->dev, "FLOW BLOCK UNBIND %s\n", netdev->name);
...
void bnxt_shutdown_tc(struct bnxt *bp)
{
	...
	netdev_dbg(bp->dev, "Unregistered indirect block notification\n");
#endif
