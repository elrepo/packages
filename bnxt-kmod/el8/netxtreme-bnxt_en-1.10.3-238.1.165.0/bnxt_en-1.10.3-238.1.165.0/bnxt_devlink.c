			   ...
			   struct netlink_ext_ack *extack)
{
	...
	if (!priv_ctx) {
#ifdef HAVE_DEVLINK_HEALTH_REPORT_EXTACK
		NL_SET_ERR_MSG_MOD(extack, "User initiated FW recovery not supported");
#endif
		return -EOPNOTSUPP;
	}
...
static void bnxt_dbg_dump_hw_ring(struct bnxt_napi *bnapi,
				  struct devlink_fmsg *fmsg)
{
	...
	bnxt_for_each_napi_tx(i, bnapi, txr) {
		...
		snprintf(str, 160,
			 "[%d.%d]: TBD{prod: %x cons: %x cp prod: %x cp cons: %x}",
			 index, i, val[0], val[1], val[2], val[3]);
		devlink_fmsg_string_put(fmsg, str);
	}
...
skip_rxr:
	...
	snprintf(str, 160,
		 "[%d]: RBD{prod: %x cons: %x cp prod: %x cp cons: %x} RBD AGG{prod: %x cons: %x cp prod: %x cp cons: %x} CAG{prod: %x cons: %x vec: %x}\n",
		 index, val[4], val[5], val[6], val[7], val[8], val[9], val[10],
		 val[11], val[12], val[13], val[14]);
	devlink_fmsg_string_put(fmsg, str);
}

/* HW states can be dumped even in ifdown state */
static void bnxt_dbg_dump_hw_states(struct bnxt *bp, struct devlink_fmsg *fmsg)
{
	...
	if (!rc) {
		snprintf(str, 160, "TDC_REG_INT_STS_0: %x\n", val[0]);
		devlink_fmsg_string_put(fmsg, str);
	}
	/* dump tdc debug bus */
	devlink_fmsg_string_put(fmsg, "TDC debug bus dump:\n");
	dbg_sel = 0x80000000;
	for (i = 0; i < 5; i++) {
		...
		if (rc)
			break;
		snprintf(str, 160, "\tdbg_sel %08x: %08x\n", dbg_sel, val[0]);
		devlink_fmsg_string_put(fmsg, str);
		dbg_sel++;
	}
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS)
	...
	/* dump tdi debug bus */
	devlink_fmsg_string_put(fmsg, "TDI debug bus dump:\n");
	dbg_sel = 0xf;
	...
	if (!rc) {
		...
		if (!rc) {
			snprintf(str, 160,
				 "\tTDI_REG_DBG_DWORD_ENABLE (%x): %08x\n",
				 dbg_sel, val[0]);
			devlink_fmsg_string_put(fmsg, str);
		}
		for (dbg_sel = 2; dbg_sel < 0x12; dbg_sel++) {
			...
			if (rc)
				break;
			snprintf(str, 160,
				 "\tTDI_REG_DBG_OUT_DATA: %08x %08x %08x %08x %08x %08x %08x %08x\n",
	...
	if (!rc) {
		snprintf(str, 160,
			 "TE_DEC_REG_PORT_CURRENT_CREDIT_REG: %x %x %x\n",
			 val[0], val[1], val[2]);
		devlink_fmsg_string_put(fmsg, str);
	}
	/* dump partial RDI debug bus */
	devlink_fmsg_string_put(fmsg, "RDI debug bus dump:\n");
	dbg_sel = 0x80000000;
	for (i = 0; i < 3; i++) {
		...
		if (rc)
			break;
		snprintf(str, 160, "\tdbg_sel %x: %08x\n", dbg_sel, val[0]);
		devlink_fmsg_string_put(fmsg, str);
		dbg_sel++;
	}
	dbg_sel = 0x80001000;
	rc = bnxt_dbg_hwrm_wr_reg(bp, RDI_REG_RDI_DEBUG_CONTROL_REG,
				  dbg_sel);
	if (!rc)
		rc = bnxt_dbg_hwrm_rd_reg(bp, RDI_REG_RDI_DEBUG_STATUS_REG, 1,
					  val);
	if (!rc) {
		snprintf(str, 160, "\tdbg_sel %x: %08x\n", dbg_sel, val[0]);
		devlink_fmsg_string_put(fmsg, str);
				  ...
				  struct devlink_fmsg *fmsg)
{
	...
	bnxt_for_each_napi_tx(j, bnapi, txr) {
		snprintf(str, 160, "[%d.%d]: tx{fw_ring: %d prod: %x cons: %x}\n",
				  ...
				  struct devlink_fmsg *fmsg)
{
	...
	snprintf(str, 160, "[%d]: cp{fw_ring: %d raw_cons: %x}\n",
	...
	for (j = 0; j < cpr->cp_ring_count; j++) {
		...
		if (!cpr2->bnapi)
			continue;
		snprintf(str, 160, "[%d.%d]: cp{fw_ring: %d raw_cons: %x}\n", i,
			 j, cpr2->cp_ring_struct.fw_ring_id, cpr2->cp_raw_cons);
		devlink_fmsg_string_put(fmsg, str);
	}
}

static void bnxt_dump_cp_ring(struct bnxt *bp, struct bnxt_cp_ring_info *cpr,
			      struct devlink_fmsg *fmsg, u8 *data, u32 len)
{
	struct bnxt_napi *bnapi = cpr->bnapi;
	u32 raw_cons, cons, dump_len = 0;
	struct tx_cmp *cmp;

	raw_cons = cpr->cp_raw_cons;
	cons = RING_CMP(raw_cons);
	cmp = &cpr->cp_desc_ring[CP_RING(cons)][CP_IDX(cons)];
	if (!TX_CMP_VALID(cmp, raw_cons))
		return;
	devlink_fmsg_u32_pair_put(fmsg, "TX CP ring", bnapi->index);
	devlink_fmsg_u32_pair_put(fmsg, "FW ring ID",
				  cpr->cp_ring_struct.fw_ring_id);
	devlink_fmsg_u32_pair_put(fmsg, "cons", cons);
	while (dump_len + sizeof(*cmp) < len && TX_CMP_VALID(cmp, raw_cons)) {
		memcpy(data + dump_len, cmp, sizeof(*cmp));
		dump_len += sizeof(struct tx_cmp);
		cons = RING_CMP(cons + 1);
		raw_cons = NEXT_RAW_CMP(raw_cons);
		cmp = &cpr->cp_desc_ring[CP_RING(cons)][CP_IDX(cons)];
	}
	devlink_fmsg_binary_pair_put(fmsg, "CP ring data", data, dump_len);
}

static void bnxt_dump_tx_ring(struct bnxt *bp, int idx,
			      struct devlink_fmsg *fmsg, u8 *data, u32 len)
{
	struct bnxt_tx_ring_info *txr = &bp->tx_ring[idx];
	u32 prod = RING_TX(bp, txr->tx_prod);
	u32 cons = RING_TX(bp, txr->tx_cons);
	struct bnxt_cp_ring_info *cpr;
	...
	if (prod == cons)
		return;
	devlink_fmsg_pair_nest_start(fmsg, "Ring dump");
	devlink_fmsg_u32_pair_put(fmsg, "TX ring", idx);
	devlink_fmsg_u32_pair_put(fmsg, "FW ring ID",
				  txr->tx_ring_struct.fw_ring_id);
	devlink_fmsg_u32_pair_put(fmsg, "prod", prod);
	devlink_fmsg_u32_pair_put(fmsg, "cons", cons);
	while (dump_len + sizeof(struct tx_bd) < len && cons != prod) {
		txbd = &txr->tx_desc_ring[TX_RING(bp, cons)][TX_IDX(cons)];
		memcpy(data + dump_len, txbd, sizeof(*txbd));
		dump_len += sizeof(*txbd);
		cons = RING_TX(bp, cons + 1);
	}
	devlink_fmsg_binary_pair_put(fmsg, "TX ring data", data, dump_len);
	bnapi = txr->bnapi;
	cpr = &bnapi->cp_ring;
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS)
		cpr = txr->tx_cpr;
	bnxt_dump_cp_ring(bp, cpr, fmsg, data, len);
	devlink_fmsg_pair_nest_end(fmsg);
}

static void bnxt_dump_all_tx_rings(struct bnxt *bp, struct devlink_fmsg *fmsg)
{
	u8 *data;
	int i;
	...
	if (!test_bit(BNXT_STATE_OPEN, &bp->state))
		return;
	data = kmalloc(BNXT_PAGE_SIZE, GFP_KERNEL);
	if (!data)
		return;
	for (i = 0; i < bp->tx_nr_rings; i++)
		bnxt_dump_tx_ring(bp, i, fmsg, data, BNXT_PAGE_SIZE);
	kfree(data);
}

static int bnxt_tx_dump(struct devlink_health_reporter *reporter,
			struct devlink_fmsg *fmsg, void *priv_ctx,
			struct netlink_ext_ack *extack)
{
	struct bnxt *bp = devlink_health_reporter_priv(reporter);
	...
	if (!priv_ctx) {
		...
		/* netdev_lock already held in devlink_health_report()
		 * context
		 */
	...
	if (test_bit(BNXT_STATE_OPEN, &bp->state)) {
		for (i = 0; i < bp->cp_nr_rings; i++) {
			...
			if (!bnapi)
				continue;
			bnxt_dump_tx_sw_state(bnapi, fmsg);
			bnxt_dump_rx_sw_state(bnapi, fmsg);
			bnxt_dump_cp_sw_state(bnapi, fmsg);
			bnxt_dbg_dump_hw_ring(bnapi, fmsg);
		}
	}
	bnxt_dbg_dump_hw_states(bp, fmsg);
	bnxt_dump_all_tx_rings(bp, fmsg);
	if (!priv_ctx) {
...
static int bnxt_tx_recover(struct devlink_health_reporter *reporter,
			   void *priv_ctx,
			   struct netlink_ext_ack *extack)
{
	struct bnxt *bp = devlink_health_reporter_priv(reporter);
	...
	/* netdev_lock already held in devlink_health_report() context */
	if (!priv_ctx) {
		rtnl_lock();
		netdev_lock(bp->dev);
	} else {
	...
	if (!test_bit(BNXT_STATE_OPEN, &bp->state)) {
#ifdef HAVE_DEVLINK_HEALTH_REPORT_EXTACK
		NL_SET_ERR_MSG_MOD(extack, "Aborted in ifdown state");
#endif
		rc = -ENETDOWN;
		goto abort_recover;
	}
	if (test_bit(BNXT_STATE_IN_FW_RESET, &bp->state)) {
#ifdef HAVE_DEVLINK_HEALTH_REPORT_EXTACK
		NL_SET_ERR_MSG_MOD(extack, "FW reset in progress");
#endif
		rc = -EBUSY;
		goto abort_recover;
	}
...
#ifdef HAVE_DEVLINK_HEALTH_REPORT_EXTACK
			NL_SET_ERR_MSG_MOD(extack, "FW recovery in progress");
#endif
			rc = -EBUSY;
			goto abort_recover;
		}
	}
	if (!bnxt_hwrm_monitor_dev_health(bp, &sts) &&
	    sts == MONITOR_DEVICE_HEALTH_RESP_STATUS_FATAL) {
#ifdef HAVE_DEVLINK_HEALTH_REPORT_EXTACK
		NL_SET_ERR_MSG_MOD(extack, "Fatal error, FW will reset");
#endif
		rc = -EIO;
		goto abort_recover;
	}

	rc = __bnxt_reset_task(bp, true);
abort_recover:
	if (!priv_ctx) {
		netdev_unlock(bp->dev);
		rtnl_unlock();
...
static const struct devlink_health_reporter_ops bnxt_dl_tx_reporter_ops = {
	.name = "tx",
	.dump = bnxt_tx_dump,
	.recover = bnxt_tx_recover,
};
...
void bnxt_dl_fw_reporters_create(struct bnxt *bp)
{
	...
	if (!bp->tx_reporter)
		bp->tx_reporter = __bnxt_dl_reporter_create(bp, &bnxt_dl_tx_reporter_ops);
...
void bnxt_dl_fw_reporters_destroy(struct bnxt *bp)
{
	...
	if (bp->tx_reporter) {
		devlink_health_reporter_destroy(bp->tx_reporter);
		bp->tx_reporter = NULL;
	}
}
...
static inline bool bnxt_sriov_is_active(struct bnxt *bp)
{
#ifdef CONFIG_BNXT_SRIOV
	bool is_active;

	mutex_lock(&bp->sriov_lock);
	is_active = (bp->pf.active_vfs || bp->sriov_cfg);
...
static int bnxt_reinit_up(struct bnxt *bp)
{
	int rc;
	...
	__acquires(&rtnl_mutex)
{
	...
	if (bnxt_sriov_is_active(bp)) {
		NL_SET_ERR_MSG_MOD(extack, "reload is unsupported when VFs are created");
		return -EOPNOTSUPP;
	}
	...
	*actions_performed = 0;
	...
	case DEVLINK_RELOAD_ACTION_FW_ACTIVATE: {
		...
		if ((~bp->fw_cap & BNXT_FW_CAP_HOT_RESET) && BNXT_CHIP_P7(bp) &&
			...
			rc = bnxt_reinit_down(bp, extack);
			if (!rc) {
				/* When UDCC is enabled firmware requires 3 seconds of
				 * downtime after issuing fw_reset
				 */
				if (BNXT_UDCC_CAP(bp))
					delay_ms = 3000;
...
enum bnxt_dl_param_id {
	...
	BNXT_DEVLINK_PARAM_ID_PREALLOC_KTLS_KIDS,	/* Explicit kTLS pre-allocation */
	BNXT_DEVLINK_PARAM_ID_PREALLOC_QUIC_KIDS,	/* Explicit QUIC pre-allocation */
	BNXT_DEVLINK_PARAM_ID_ENABLE_VF_ROCE,
	BNXT_DEVLINK_PARAM_ID_DISABLE_VF_ROCE,
	BNXT_DEVLINK_PARAM_ID_ULP_NUM_RINGS,
	BNXT_DEVLINK_PARAM_ID_ULP_NUM_VNICS,
};
...
/* Shared internal function for KID pre-allocation */
static int bnxt_dl_prealloc_kid_internal(struct bnxt *bp,
					 struct bnxt_tls_info *tls_info,
					 enum bnxt_crypto_type tls_type,
					 u32 requested)
{
	u32 needed, pre_alloc_kids_num_batch;
	struct bnxt_kctx *kctx;
	int rc = 0, i;

	atomic_inc(&tls_info->pending);
	kctx = &tls_info->tck;

	if (requested <= kctx->total_alloc)
		goto done;

	needed = requested - kctx->total_alloc;
	pre_alloc_kids_num_batch = DIV_ROUND_UP(needed, BNXT_KID_BATCH_SIZE);

	for (i = 0; i < pre_alloc_kids_num_batch; i++) {
		u32 alloc_sz = min(needed - i * BNXT_KID_BATCH_SIZE,
				   (u32)BNXT_KID_BATCH_SIZE);
		rc = bnxt_hwrm_key_ctx_alloc(bp, kctx, alloc_sz, NULL, tls_type);
		if (rc) {
			netdev_warn(bp->dev,
				    "%s: Preallocated %d TX KIDs only, requested: %d\n",
				    tls_type == BNXT_CRYPTO_TYPE_KTLS ? "kTLS" : "QUIC",
				    i * BNXT_KID_BATCH_SIZE, requested);
			break;
		}
	}

done:
	atomic_dec(&tls_info->pending);
	return rc;
}

/* kTLS-specific devlink parameter functions */
static int bnxt_dl_prealloc_ktls_kid_param_get(struct devlink *dl, u32 id,
					       struct devlink_param_gset_ctx *ctx,
					       struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	ctx->val.vu32 = bp->ktls_info->tck.total_alloc;

	return 0;
}

static int bnxt_dl_prealloc_ktls_kid_param_set(struct devlink *dl, u32 id,
					       struct devlink_param_gset_ctx *ctx,
					       struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	return bnxt_dl_prealloc_kid_internal(bp, bp->ktls_info,
					     BNXT_CRYPTO_TYPE_KTLS,
					     ctx->val.vu32);
}

static int bnxt_dl_prealloc_ktls_kid_param_validate(struct devlink *dl, u32 id,
						    union devlink_param_value val,
						    struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!val.vu32) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "prealloc_ktls_tx_kids value must be greater than zero");
		return -EINVAL;
	}

	kctx = &bp->ktls_info->tck;

	if (val.vu32 <= kctx->total_alloc) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested number of kTLS KIDs are already allocated");
		return -EINVAL;
	}

	if (val.vu32 > kctx->max_ctx) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested kTLS KIDs exceed capacity");
		return -ENOSPC;
	}
...
/* QUIC-specific devlink parameter functions */
static int bnxt_dl_prealloc_quic_kid_param_get(struct devlink *dl, u32 id,
					       struct devlink_param_gset_ctx *ctx,
					       struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	ctx->val.vu32 = bp->quic_info->tck.total_alloc;

	return 0;
}

static int bnxt_dl_prealloc_quic_kid_param_set(struct devlink *dl, u32 id,
					       struct devlink_param_gset_ctx *ctx,
					       struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	return bnxt_dl_prealloc_kid_internal(bp, bp->quic_info,
					     BNXT_CRYPTO_TYPE_QUIC,
					     ctx->val.vu32);
}

static int bnxt_dl_prealloc_quic_kid_param_validate(struct devlink *dl, u32 id,
						    union devlink_param_value val,
						    struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!val.vu32) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "prealloc_quic_tx_kids value must be greater than zero");
		return -EINVAL;
	}

	kctx = &bp->quic_info->tck;

	if (val.vu32 <= kctx->total_alloc) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested number of QUIC KIDs are already allocated");
		return -EINVAL;
	}

	if (val.vu32 > kctx->max_ctx) {
		__NL_SET_ERR_MSG_MOD(extack,
				     "Requested QUIC KIDs exceed capacity");
		return -ENOSPC;
	}
...
static int bnxt_dl_vf_roce_set(struct bnxt *bp, u32 vf_idx, bool enable,
			       struct netlink_ext_ack *extack)
{
	struct bnxt_vf_info *vf_tbl, *vf;
	struct hwrm_func_cfg_input *req;
	int rc;

	mutex_lock(&bp->sriov_lock);

	vf_tbl = rcu_dereference_protected(bp->pf.vf,
					   lockdep_is_held(&bp->sriov_lock));

	if (!vf_tbl || vf_idx >= bp->pf.active_vfs) {
		mutex_unlock(&bp->sriov_lock);
		__NL_SET_ERR_MSG_MOD(extack, "Invalid VF index");
		return -EINVAL;
	}
	...
	if (enable)
		req->flags2 |= cpu_to_le32(FUNC_CFG_REQ_FLAGS2_ROCE_ENABLE_DISABLE_ENABLE);
	else
		req->flags2 |= cpu_to_le32(FUNC_CFG_REQ_FLAGS2_ROCE_ENABLE_DISABLE_DISABLE);
	...
	if (rc)
		__NL_SET_ERR_MSG_MOD(extack, "Failed to change VF configuration");
...
static int bnxt_dl_enable_vf_roce_param_get(struct devlink *dl, u32 id,
					    struct devlink_param_gset_ctx *ctx,
					    struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	ctx->val.vu32 = U32_MAX;

	return 0;
}

static int bnxt_dl_enable_vf_roce_param_set(struct devlink *dl, u32 id,
					    struct devlink_param_gset_ctx *ctx,
					    struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!BNXT_PF(bp)) {
		__NL_SET_ERR_MSG_MOD(extack, "VF RoCE enable is only supported on PF");
		return -EOPNOTSUPP;
	}

	rc = bnxt_dl_vf_roce_set(bp, ctx->val.vu32, true, extack);

	return rc;
}

static int bnxt_dl_enable_vf_roce_param_validate(struct devlink *dl, u32 id,
						 union devlink_param_value val,
						 struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);

	if (!(bp->flags & BNXT_FLAG_ROCE_CAP) || !BNXT_PF(bp)) {
		__NL_SET_ERR_MSG_MOD(extack, "VF RoCE disable requires RDMA enabled on PF");
		return -EOPNOTSUPP;
	}

	if (val.vu32 >= bp->pf.active_vfs) {
		__NL_SET_ERR_MSG_MOD(extack, "Invalid VF index");
		return -EINVAL;
	}
...
static int bnxt_dl_disable_vf_roce_param_get(struct devlink *dl, u32 id,
					     struct devlink_param_gset_ctx *ctx,
					     struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
...
static int bnxt_dl_disable_vf_roce_param_set(struct devlink *dl, u32 id,
					     struct devlink_param_gset_ctx *ctx,
					     struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!BNXT_PF(bp)) {
		__NL_SET_ERR_MSG_MOD(extack, "VF RoCE disable is only supported on PF");
		return -EOPNOTSUPP;
	}

	rc = bnxt_dl_vf_roce_set(bp, ctx->val.vu32, false, extack);

	return rc;
}

static int bnxt_dl_disable_vf_roce_param_validate(struct devlink *dl, u32 id,
						  union devlink_param_value val,
						  struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
...
static u16 bnxt_get_l2_rx_rings_in_use(struct bnxt *bp)
{
	if (bp->flags & BNXT_FLAG_AGG_RINGS)
		return bp->rx_nr_rings << 1;
	return bp->rx_nr_rings;
}

static u16 bnxt_get_l2_tx_rings_in_use(struct bnxt *bp)
{
	return bp->tx_nr_rings;
}

static u16 bnxt_get_l2_cp_rings_in_use(struct bnxt *bp)
{
	if (BNXT_CHIP_P5_PLUS(bp))
		return bp->tx_nr_rings + bp->rx_nr_rings;
	return bp->cp_nr_rings;
}

static u16 bnxt_get_l2_stat_ctxs_in_use(struct bnxt *bp)
{
	return bp->cp_nr_rings;
}

static u16 bnxt_get_l2_vnics_in_use(struct bnxt *bp)
{
	return bp->nr_vnics;
}

static u16 bnxt_get_l2_rss_ctxs_in_use(struct bnxt *bp)
{
	return bp->rsscos_nr_ctxs;
}

struct bnxt_ulp_resc_desc {
	const char *name;
	bool p5_only;
	u16 hw_max_off;
	u16 (*get_l2_usage)(struct bnxt *bp);
	int (*get_mpc_usage)(struct bnxt *bp);
};

static const struct bnxt_ulp_resc_desc bnxt_ulp_resc_rings[] = {
	{
		.name = "RX",
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_rx_rings) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_rx_rings_in_use,
	},
	{
		.name = "TX",
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_tx_rings) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_tx_rings_in_use,
		.get_mpc_usage = bnxt_mpc_tx_rings_in_use,
	},
	{
		.name = "CP",
		.p5_only = true,
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_cp_rings) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_cp_rings_in_use,
		.get_mpc_usage = bnxt_mpc_cp_rings_in_use,
	},
	{
		.name = "STAT",
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_stat_ctxs) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_stat_ctxs_in_use,
	},
};

static const struct bnxt_ulp_resc_desc bnxt_ulp_resc_vnics[] = {
	{
		.name = "VNIC",
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_vnics) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_vnics_in_use,
	},
	{
		.name = "RSS",
		.hw_max_off = offsetof(struct bnxt_hw_resc, max_rsscos_ctxs) / sizeof(u16),
		.get_l2_usage = bnxt_get_l2_rss_ctxs_in_use,
	},
};

static inline u16 bnxt_calc_avail_resc(u16 max, u16 l2, int mpc)
{
	if (max < (l2 + mpc))
		return 0;
	return max - l2 - mpc;
...
static int __bnxt_dl_ulp_num_rings_validate(struct bnxt *bp,
					    struct netlink_ext_ack *extack,
					    u16 req)
{
	u16 avail;
	...
	BUILD_BUG_ON(ARRAY_SIZE(bnxt_ulp_resc_rings) > __BNXT_ULP_RES_MAX);
	for (i = 0; i < ARRAY_SIZE(bnxt_ulp_resc_rings); i++) {
		const struct bnxt_ulp_resc_desc *res = &bnxt_ulp_resc_rings[i];
		u16 l2_usage = res->get_l2_usage(bp);
		u16 hw_max = *((u16 *)(&bp->hw_resc) + res->hw_max_off);
		int mpc_usage = 0;

		if (res->p5_only && !BNXT_CHIP_P5_PLUS(bp))
			continue;
		if (res->get_mpc_usage)
			mpc_usage = res->get_mpc_usage(bp);

		avail = bnxt_calc_avail_resc(hw_max, l2_usage, mpc_usage);

		if (req > avail) {
			__NL_SET_ERR_MSG_FMT_MOD(extack, "%s: Req:%u / Avail:%u",
						 res->name, req, avail);
			return -EINVAL;
		}
	}
	...
	if (BNXT_CHIP_P5_PLUS(bp)) {
		...
		max_func_irqs = bnxt_get_max_func_irqs(bp);
		avail = max_func_irqs - bnxt_get_ulp_msix_num_in_use(bp) - req;

		if (bnxt_get_l2_stat_ctxs_in_use(bp) > avail) {
			__NL_SET_ERR_MSG_FMT_MOD(extack, "IRQ: Req:%u / Avail:%u",
						 req, avail);
			return -EINVAL;
		}
	}
...
static int __bnxt_dl_ulp_num_vnics_validate(struct bnxt *bp,
					    struct netlink_ext_ack *extack,
					    u16 req)
{
	u16 avail;
	...
	BUILD_BUG_ON(ARRAY_SIZE(bnxt_ulp_resc_vnics) > __BNXT_ULP_RES_MAX);
	for (i = 0; i < ARRAY_SIZE(bnxt_ulp_resc_vnics); i++) {
		const struct bnxt_ulp_resc_desc *res = &bnxt_ulp_resc_vnics[i];
		u16 l2_usage = res->get_l2_usage(bp);
		u16 hw_max = *((u16 *)(&bp->hw_resc) + res->hw_max_off);
		int mpc_usage = 0;
...
static int bnxt_dl_ulp_num_rings_param_validate(struct devlink *dl, u32 id,
						union devlink_param_value val,
						struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!bp->edev_rdma) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "RDMA is not enabled on the function");
		return -EOPNOTSUPP;
	}

	return __bnxt_dl_ulp_num_rings_validate(bp, extack, val.vu16);
}

static int bnxt_dl_ulp_num_rings_param_get(struct devlink *dl, u32 id,
					   struct devlink_param_gset_ctx *ctx,
					   struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!bp->edev_rdma)
		...
		ctx->val.vu16 = bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX);

	return 0;
}

static int bnxt_dl_ulp_num_rings_param_set(struct devlink *dl, u32 id,
					   struct devlink_param_gset_ctx *ctx,
					   struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	u16 requested_rings = ctx->val.vu16;
	...
	/* Validate request one more time due to possible race condition */
	rc = __bnxt_dl_ulp_num_rings_validate(bp, NULL, requested_rings);
	if (rc) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "ulp_num_rings: Validation failed during SET");
		goto unlock;
	}

	physical_rx = bnxt_get_l2_rx_rings_in_use(bp);
	logical_rx = bp->rx_nr_rings;
	hwr.rx = physical_rx + requested_rings;
	hwr.tx = bnxt_get_l2_tx_rings_in_use(bp) + requested_rings;
	if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
		hwr.cp_p5 = bnxt_get_l2_cp_rings_in_use(bp);
		hwr.cp_p5 += bnxt_mpc_cp_rings_in_use(bp);
		hwr.cp_p5 += 2 * requested_rings;
		hwr.tx += bnxt_mpc_tx_rings_in_use(bp);
		hwr.cp = bnxt_nq_rings_in_use(bp);
	} else {
		hwr.cp = logical_rx + bnxt_get_l2_tx_rings_in_use(bp);
	}
	hwr.grp = logical_rx;

	hwr.stat = bnxt_get_l2_stat_ctxs_in_use(bp);
	hwr.stat += bnxt_calc_dflt_ulp_stat_ctxs_reserved(bp);
	hwr.stat += requested_rings;

	/* Do not allow resources to be reduced without unloading RoCE driver */
	if (requested_rings < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RX) &&
	    bnxt_ulp_registered(bp->edev_rdma)) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "ulp_num_rings: Unable to reduce resources. Unload RoCE driver to modify.");
		rc = -EINVAL;
		goto unlock;
	}
	bnxt_hwrm_reserve_rings(bp, &hwr);

	/* If we only got a partial allocation of the resources we requested,
	 * keep the allocation, but don't update the resource array, so RoCE
	 * will use the existing values.
	 */
	if ((bp->hw_resc.resv_rx_rings != hwr.rx) ||
	    (bp->hw_resc.resv_tx_rings != hwr.tx) ||
	    (bp->hw_resc.resv_cp_rings != hwr.cp_p5) ||
	    (bp->hw_resc.resv_stat_ctxs != hwr.stat)) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "Failed to reserve requested rings\n");
		rc = -ENOMEM;
		goto unlock;
	}

	/* Rings reservation succeeded, update the resource array */
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_RX,
			  hwr.rx - physical_rx);
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_TX,
			  hwr.tx - bnxt_get_l2_tx_rings_in_use(bp) -
			  bnxt_mpc_tx_rings_in_use(bp));
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_CP,
			  hwr.cp_p5 - bnxt_get_l2_cp_rings_in_use(bp) -
			  bnxt_mpc_cp_rings_in_use(bp));
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_STAT_CTX,
			  hwr.stat - bnxt_get_l2_stat_ctxs_in_use(bp));
unlock:
#ifdef HAVE_NETDEV_LOCK
	netdev_unlock(bp->dev);
...
static int bnxt_dl_ulp_num_vnics_param_validate(struct devlink *dl, u32 id,
						union devlink_param_value val,
						struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	return __bnxt_dl_ulp_num_vnics_validate(bp, extack, val.vu16);
}

static int bnxt_dl_ulp_num_vnics_param_get(struct devlink *dl, u32 id,
					   struct devlink_param_gset_ctx *ctx,
					   struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	...
	if (!bp->edev_rdma)
		...
		ctx->val.vu16 = bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC);

	return 0;
}

static int bnxt_dl_ulp_num_vnics_param_set(struct devlink *dl, u32 id,
					   struct devlink_param_gset_ctx *ctx,
					   struct netlink_ext_ack *extack)
{
	struct bnxt *bp = bnxt_get_bp_from_dl(dl);
	u16 requested_vnics = ctx->val.vu16;
	struct bnxt_hw_rings hwr = {0};
	int rc;
	...
	/* Validate request one more time due to possible race condition */
	rc = __bnxt_dl_ulp_num_vnics_validate(bp, NULL, requested_vnics);
	if (rc) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "ulp_num_vnics: Validation failed during SET");
		goto unlock;
	}
	hwr.rx = bnxt_get_l2_rx_rings_in_use(bp);
	hwr.vnic = bnxt_get_total_vnics(bp, hwr.rx);
	hwr.vnic -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC);
	hwr.vnic += requested_vnics;
	hwr.grp = bp->rx_nr_rings;
	hwr.rss_ctx = bnxt_get_total_rss_ctxs(bp, &hwr);
	hwr.rss_ctx -= bnxt_get_ulp_resc(bp, BNXT_ULP_RES_RSS_CTX);
	hwr.rss_ctx += requested_vnics;
	hwr.rx = 0;
	hwr.grp = 0;

	/* Do not allow resources to be reduced without unloading RoCE driver */
	if (requested_vnics < bnxt_get_ulp_resc(bp, BNXT_ULP_RES_VNIC) &&
	    bnxt_ulp_registered(bp->edev_rdma)) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "ulp_num_vnics: Unable to reduce resources. Unload RoCE driver to modify.");
		rc = -EINVAL;
		goto unlock;
	}
	bnxt_hwrm_reserve_rings(bp, &hwr);
	 ...
	 */
	if ((bp->hw_resc.resv_vnics != hwr.vnic) ||
	    (bp->hw_resc.resv_rsscos_ctxs != hwr.rss_ctx)) {
		__NL_SET_ERR_MSG_FMT_MOD(extack,
					 "Failed to reserve requested vnics/rss\n");
		rc = -ENOMEM;
		goto unlock;
	}

	/* Reservation succeeded, update the resource array */
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_VNIC,
			  hwr.vnic - bnxt_get_l2_vnics_in_use(bp));
	bnxt_set_ulp_resc(bp, BNXT_ULP_RES_RSS_CTX,
			  hwr.rss_ctx - bnxt_get_l2_rss_ctxs_in_use(bp));
unlock:
#ifdef HAVE_NETDEV_LOCK
	netdev_unlock(bp->dev);
#endif
...
static const struct devlink_param bnxt_dl_params[] = {
	...
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_TRUFLOW,
			     ...
			     NULL),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_PREALLOC_KTLS_KIDS,
			     "prealloc_ktls_tx_kids", DEVLINK_PARAM_TYPE_U32,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_prealloc_ktls_kid_param_get,
			     bnxt_dl_prealloc_ktls_kid_param_set,
			     bnxt_dl_prealloc_ktls_kid_param_validate),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_PREALLOC_QUIC_KIDS,
			     "prealloc_quic_tx_kids", DEVLINK_PARAM_TYPE_U32,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_prealloc_quic_kid_param_get,
			     bnxt_dl_prealloc_quic_kid_param_set,
			     bnxt_dl_prealloc_quic_kid_param_validate),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_ENABLE_VF_ROCE,
			     "enable_vf_roce", DEVLINK_PARAM_TYPE_U32,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_enable_vf_roce_param_get, bnxt_dl_enable_vf_roce_param_set,
			     bnxt_dl_enable_vf_roce_param_validate),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_DISABLE_VF_ROCE,
			     "disable_vf_roce", DEVLINK_PARAM_TYPE_U32,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_disable_vf_roce_param_get, bnxt_dl_disable_vf_roce_param_set,
			     bnxt_dl_disable_vf_roce_param_validate),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_ULP_NUM_RINGS,
			     "ulp_num_rings", DEVLINK_PARAM_TYPE_U16,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_ulp_num_rings_param_get, bnxt_dl_ulp_num_rings_param_set,
			     bnxt_dl_ulp_num_rings_param_validate),
	DEVLINK_PARAM_DRIVER(BNXT_DEVLINK_PARAM_ID_ULP_NUM_VNICS,
			     "ulp_num_vnics", DEVLINK_PARAM_TYPE_U16,
			     BIT(DEVLINK_PARAM_CMODE_RUNTIME),
			     bnxt_dl_ulp_num_vnics_param_get, bnxt_dl_ulp_num_vnics_param_set,
			     bnxt_dl_ulp_num_vnics_param_validate),
#ifdef HAVE_REMOTE_DEV_RESET
