...
static void __bnxt_tx_int_xdp(struct bnxt *bp, struct bnxt_tx_ring_info *txr)
{
	...
	if (txr->xsk_pool && xsk_tx) {
		...
		bnapi->cp_ring.sw_stats->xsk_stats.xsk_tx_completed += xsk_tx;
	}
		  ...
		  struct xdp_frame **frames, u32 flags)
{
	...
	if (!test_bit(BNXT_STATE_OPEN, &bp->state) ||
	    !BNXT_TX_XDP_RINGS_PER_GRP(bp) || !xdp_prog)
		return -EINVAL;

	ring = smp_processor_id() % BNXT_TX_XDP_RINGS_PER_GRP(bp);
	ring += BNXT_TX_XDP_RDIR_RING_BASE(bp);
	txr = &bp->tx_ring[ring];
...
static int bnxt_xdp_set(struct bnxt *bp, struct bpf_prog *prog)
{
	...
	if (prog)
		tx_xdp = bp->rx_nr_rings * BNXT_XDP_TX_PER_NAPI;
