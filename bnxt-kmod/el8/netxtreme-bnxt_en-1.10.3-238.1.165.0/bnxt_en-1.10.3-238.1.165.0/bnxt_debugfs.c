...
static ssize_t debugfs_mpc_table_read(struct file *filep, char __user *buffer,
				      size_t count, loff_t *ppos)
{
	return bnxt_tfc_em_mpc_read_decode((struct bnxt *)(filep->private_data),
					   buffer, count, ppos);
}

static const struct file_operations debugfs_mpc_table_read_fops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = debugfs_mpc_table_read,
};

static void bnxt_debugfs_mpc_init(struct bnxt *bp)
{
	struct dentry *mpc_dir;

	if (!bp->debugfs_pdev)
		return;

	mpc_dir = debugfs_create_dir("mpc", bp->debugfs_pdev);
	if (!mpc_dir) {
		pr_err("Failed to create debugfs entry %s/mpc\n",
		       pci_name(bp->pdev));
		return;
	}

	debugfs_create_u8("table_scope", 0644, mpc_dir,
			  &bp->mpc_debug.table_scope);
	debugfs_create_u32("entry_id", 0644, mpc_dir,
			   &bp->mpc_debug.entry_id);
	/* Initialize direction to TX (1) by default */
	bp->mpc_debug.direction = CFA_DIR_TX;
	debugfs_create_u8("direction", 0644, mpc_dir,
			  &bp->mpc_debug.direction);
	/* Initialize decode_mode to raw (2) by default */
	bp->mpc_debug.decode_mode = 2;
	debugfs_create_u8("decode_mode", 0644, mpc_dir,
			  &bp->mpc_debug.decode_mode);
	debugfs_create_file("read", 0444, mpc_dir, bp,
			    &debugfs_mpc_table_read_fops);
}
