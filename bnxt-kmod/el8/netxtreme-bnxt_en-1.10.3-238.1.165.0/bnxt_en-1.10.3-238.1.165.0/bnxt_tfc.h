...
struct bnxt_tfc_mpc_info {
	...
	atomic_t		pending;
	/* Reference count of concurrent tfc_tbl_scope_func_reset() calls.
	 * Each invocation increments with atomic_inc() + smp_mb__after_atomic()
	 * before issuing MPC commands, and decrements with atomic_dec() at the
	 * cleanup label. MPC completion handlers use atomic_read_acquire() > 0
	 * to decide the SCOPE_ERR log level. Using a counter rather than a
	 * flag allows concurrent VF FLRs to overlap without one call clearing
	 * the other's in-progress marker early.
	 */
