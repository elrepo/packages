			       ...
			       u32 *dump_len)
{
	...
	*dump_len = seg_hdr_len + ver_get_resp_len;
	...
	if (BNXT_TRUFLOW_EN(bp)) {
		tfc_rc = tfc_corefile_bs_size(bp, &bstore_bytes);
		if (!tfc_rc && buf && bstore_bytes) {
			if (!(tfc_corefile_bs_save(bp, buf + offset, &bstore_bytes))) {
				offset += seg_hdr_len + bstore_bytes;
				tf_seg++;
			}
		}
		if (!tfc_rc && bstore_bytes)
