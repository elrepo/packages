...
int bnxt_hwrm_fwd_async_event_cmpl(struct bnxt *bp,
...
static void bnxt_hwrm_roce_sriov_cfg(struct bnxt *bp, int num_vfs)
{
	...
	if (!BNXT_ROCE_VF_DYN_ALLOC_CAP(bp) &&
	    bp->pf.vf_resv_strategy == BNXT_VF_RESV_STRATEGY_MINIMAL &&
	    bp->fw_cap_ext & BNXT_FW_CAP_VF_ASYM_RESC_ALLOC_SUPPORT) {
		cfg_req->roce_max_av_per_vf = cpu_to_le32(1);
		cfg_req->roce_max_cq_per_vf = cpu_to_le32(1);
		cfg_req->roce_max_mrw_per_vf = cpu_to_le32(1);
		cfg_req->roce_max_qp_per_vf = cpu_to_le32(1);
		cfg_req->roce_max_srq_per_vf = cpu_to_le32(1);
	} else {
...
static int bnxt_hwrm_func_vf_resc_cfg(struct bnxt *bp, int num_vfs, bool reset)
{
	...
	vf_rx_rings = hw_resc->max_rx_rings - bnxt_total_rx_rings(bp);
	...
	if (pf->vf_resv_strategy == BNXT_VF_RESV_STRATEGY_MINIMAL ||
		...
		if (bp->fw_cap_ext & BNXT_FW_CAP_VF_ASYM_RESC_ALLOC_SUPPORT) {
			req->max_msix = min + 2; /* RoCE (2) */
			vf_msix = req->max_msix * num_vfs;
		}
	} else {
		vf_cp_rings /= num_vfs;
		vf_tx_rings /= num_vfs;
	...
	for (i = 0; i < num_vfs; i++) {
		...
		vf->max_rsscos_ctx = le16_to_cpu(req->max_rsscos_ctx);
		vf->max_nqs = le16_to_cpu(req->max_msix);
	}
...
static int bnxt_sriov_enable(struct bnxt *bp, int *num_vfs)
{
	...
	if (!bnxt_ulp_registered(bp->edev_rdma)) {
		...
		bnxt_set_ulp_resc(bp, BNXT_ULP_RES_MSIX, 0);
	...
	/* switchdev mode only supported if devlink parameter exists */
#ifndef CONFIG_VF_REPS
	return 0;
#else
...
static int bnxt_hwrm_oem_cmd(struct bnxt *bp, struct bnxt_vf_info *vf)
{
	...
	if (oem_cmd->oem_id == 0x14e4 &&
		...
		if (!tfcp || !tfcp->bp) {
			netdev_dbg(bp->dev,
				   "OEM TruFlow cmd rejected: TFC not initialized\n");
			return -EOPNOTSUPP;
		}

		/* Reject requests where the VF-supplied hdr.fid does not match
		 * the firmware-assigned FID, preventing cross-tenant FID spoofing.
		 */
		if (((struct tfc_vf2pf_hdr *)oem_cmd->oem_data)->fid != vf->fw_fid) {
			netdev_dbg(bp->dev,
				   "OEM TruFlow cmd rejected: fid mismatch (got 0x%x, expected 0x%x)\n",
				   ((struct tfc_vf2pf_hdr *)oem_cmd->oem_data)->fid,
				   vf->fw_fid);
			return -EPERM;
		}
...
static int bnxt_vf_req_validate_snd(struct bnxt *bp, struct bnxt_vf_info *vf)
{
	...
	case HWRM_OEM_CMD:
		...
		/* Send error response to VF if error occurs.
		 * Caller sees fwd_err_resp result, not original error.
		 */
		if (rc)
...
#ifdef HAVE_SRIOV_SET_MSIX
u32 bnxt_sriov_get_vf_total_msix(struct pci_dev *pdev)
{
	struct net_device *dev = pci_get_drvdata(pdev);
	struct bnxt *bp = netdev_priv(dev);
	...
	return bp->hw_resc.max_nqs - bnxt_min_nq_rings_in_use(bp);
}

int bnxt_sriov_set_msix_vec_count(struct pci_dev *vf_dev, int count)
{
	struct hwrm_func_vf_resource_cfg_input *req;
	...
	pf = &bp->pf;
	if (count > pf->max_msix_vfs)
		return -EINVAL;

	hw_resc = &bp->hw_resc;
	vf_idx = pci_iov_vf_id(vf_dev);
	if (vf_idx < 0)
		return vf_idx;

	mutex_lock(&bp->sriov_lock);

	if (vf_idx >= pf->active_vfs) {
		rc = -EINVAL;
		goto exit;
	}

	vf = rcu_dereference_protected(pf->vf,
				       lockdep_is_held(&bp->sriov_lock));
	if (!vf) {
		rc = -EINVAL;
		goto exit;
	...
	vf = &vf[vf_idx];
	if (count > vf->max_nqs &&
	    ((count - vf->max_nqs) > bnxt_sriov_get_vf_total_msix(bp->pdev))) {
		rc = -EINVAL;
		goto exit;
	}
	...
	req->max_cmpl_rings = cpu_to_le16(count);
	req->min_cmpl_rings = cpu_to_le16(count);
	req->max_tx_rings = cpu_to_le16(count);
	req->min_tx_rings = cpu_to_le16(count);
	req->max_rx_rings = cpu_to_le16(count);
	req->min_rx_rings = cpu_to_le16(count);
	req->max_l2_ctxs = cpu_to_le16(BNXT_VF_MAX_L2_CTX);
	req->min_l2_ctxs = req->max_l2_ctxs;
	req->max_vnics = cpu_to_le16(count);
	req->min_vnics = cpu_to_le16(count);
	req->max_stat_ctx = cpu_to_le16(count);
	req->min_stat_ctx = cpu_to_le16(count);
	req->max_rsscos_ctx = cpu_to_le16(count);
	req->min_rsscos_ctx = cpu_to_le16(count);
	req->max_msix = cpu_to_le16(count);
clear_resv:
	rc = hwrm_req_send(bp, req);
	if (rc)
		goto exit;
	...
	if (vf->max_nqs) {
		hw_resc->max_tx_rings += vf->max_tx_rings;
		hw_resc->max_rx_rings += vf->max_rx_rings;
		hw_resc->max_cp_rings += vf->min_cp_rings;
		hw_resc->max_rsscos_ctxs += vf->max_rsscos_ctx;
		hw_resc->max_stat_ctxs += vf->min_stat_ctxs;
		hw_resc->max_vnics += vf->min_vnics;
		hw_resc->max_nqs += vf->max_nqs;
	}
	...
	if (!count) {
		...
		vf->min_tx_rings = le16_to_cpu(req->min_tx_rings);
		vf->max_tx_rings = vf->min_tx_rings;
		vf->min_rx_rings = le16_to_cpu(req->min_rx_rings);
		vf->max_rx_rings = vf->min_rx_rings;
		vf->min_cp_rings = le16_to_cpu(req->min_cmpl_rings);
		vf->min_stat_ctxs = le16_to_cpu(req->min_stat_ctx);
		vf->min_ring_grps = le16_to_cpu(req->min_hw_ring_grps);
		vf->min_vnics = le16_to_cpu(req->min_vnics);
		...
		hw_resc->max_tx_rings -= count;
		hw_resc->max_rx_rings -= count;
		hw_resc->max_cp_rings -= count;
		hw_resc->max_rsscos_ctxs -= count;
		hw_resc->max_stat_ctxs -= count;
		hw_resc->max_vnics -= count;
		hw_resc->max_nqs -= count;
	}
exit:
	mutex_unlock(&bp->sriov_lock);
	return rc;
