...
static int fwctl_cmd_info(struct fwctl_ucmd *ucmd)
{
	...
	ret = ucmd_respond(ucmd, sizeof(*cmd));
out:
	kfree(driver_info);
	return ret;
}
...
static int fwctl_cmd_rpc(struct fwctl_ucmd *ucmd)
{
	...
	if (copy_to_user(u64_to_user_ptr(cmd->out), outbuf,
			 min_t(__u32, cmd->out_len, out_len))) {
		ret = -EFAULT;
		goto free_buffers;
	}
...
static int fwctl_fops_open(struct inode *inode, struct file *filp)
{
	...
	down_read(&fwctl->registration_lock);

	if (!fwctl->ops) {
		up_read(&fwctl->registration_lock);
		return -ENODEV;
	}
	...
	mutex_lock(&fwctl->uctx_list_lock);
	list_add_tail(&uctx->uctx_list_entry, &fwctl->uctx_list);
	mutex_unlock(&fwctl->uctx_list_lock);
...
{
	struct fwctl_device *fwctl = kzalloc(size, GFP_KERNEL);
	int devnum;
...
struct fwctl_device *_fwctl_alloc_device(struct device *parent,
					 ...
					 size_t size)
{
	struct fwctl_device *fwctl = _alloc_device(parent, ops, size);
...
void fwctl_unregister(struct fwctl_device *fwctl)
{
	...
	down_write(&fwctl->registration_lock);
	mutex_lock(&fwctl->uctx_list_lock);
	 ...
	 */
	fwctl->ops = NULL;
	up_write(&fwctl->registration_lock);
}

/* EXPORT_SYMBOL_NS_GPL(fwctl_unregister, "FWCTL"); */

int __init fwctl_init(void)
{
	int ret;
