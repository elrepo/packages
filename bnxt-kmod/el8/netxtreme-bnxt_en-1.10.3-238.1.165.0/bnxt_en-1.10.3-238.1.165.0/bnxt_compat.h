...
#if defined(HAVE_XDP_RXQ_INFO) || defined(HAVE_XDP_UPDATE_SKB_FRAGS_INFO)
#include <net/xdp.h>
#endif
...
#define BNXT_UDP_TUNNEL_NIC_INFO_OPEN_ONLY \
	((UDP_TUNNEL_NIC_INFO_OPEN_ONLY) | (UDP_TUNNEL_NIC_INFO_MAY_SLEEP))
#else
#define BNXT_UDP_TUNNEL_NIC_INFO_OPEN_ONLY UDP_TUNNEL_NIC_INFO_OPEN_ONLY
#endif
...
#if defined(DEFAULT_MAX_NUM_RSS_QUEUES) || !defined(HAVE_NETIF_GET_DEFAULT_RSS)
#include <linux/crash_dump.h>

/* Use the latest formula in netif_get_num_default_rss_queues() (>= 5.18 kernel) */
static inline int bnxt_compat_adjust_dflt_rss_queues(int dflt_queues)
{
	cpumask_var_t cpus;
	int cpu, count = 0;
...
#endif /* DEFAULT_MAX_NUM_RSS_QUEUES) || !HAVE_NETIF_GET_DEFAULT_RSS */
...
#ifndef HAVE_PCI_DEV_LOCK
static inline void pci_dev_lock(struct pci_dev *dev)
{
...
#ifndef HAVE_PCI_DEV_UNLOCK
static inline void pci_dev_unlock(struct pci_dev *dev)
{
...
#define bnxt_tx_dump(reporter, fmsg, priv_ctx, extack)	\
	bnxt_tx_dump(reporter, fmsg, priv_ctx)
#define bnxt_tx_recover(reporter, priv_ctx, extack)	\
	bnxt_tx_recover(reporter, priv_ctx)
...
#if defined(HAVE_DEVLINK) && !defined(HAVE_DEVLINK_PARAM_GET_EXTACK)
#define bnxt_dl_nvm_param_get(dl, id, ctx, extack)	\
	bnxt_dl_nvm_param_get(dl, id, ctx)

#define bnxt_remote_dev_reset_get(dl, id, ctx, extack)	\
	bnxt_remote_dev_reset_get(dl, id, ctx)

#define bnxt_dl_truflow_param_get(dl, id, ctx, extack)	\
	bnxt_dl_truflow_param_get(dl, id, ctx)

#define bnxt_dl_prealloc_ktls_kid_param_get(dl, id, ctx, extack)	\
	bnxt_dl_prealloc_ktls_kid_param_get(dl, id, ctx)

#define bnxt_dl_prealloc_quic_kid_param_get(dl, id, ctx, extack)	\
	bnxt_dl_prealloc_quic_kid_param_get(dl, id, ctx)

#define bnxt_dl_enable_vf_roce_param_get(dl, id, ctx, extack)	\
	bnxt_dl_enable_vf_roce_param_get(dl, id, ctx)

#define bnxt_dl_disable_vf_roce_param_get(dl, id, ctx, extack)	\
	bnxt_dl_disable_vf_roce_param_get(dl, id, ctx)

#define bnxt_dl_ulp_num_rings_param_get(dl, id, ctx, extack)	\
	bnxt_dl_ulp_num_rings_param_get(dl, id, ctx)

#define bnxt_dl_ulp_num_vnics_param_get(dl, id, ctx, extack)	\
	bnxt_dl_ulp_num_vnics_param_get(dl, id, ctx)
#endif
...
#define bnxt_dl_prealloc_ktls_kid_param_set(dl, id, ctx, extack)	\
	bnxt_dl_prealloc_ktls_kid_param_set(dl, id, ctx)

#define bnxt_dl_prealloc_quic_kid_param_set(dl, id, ctx, extack)	\
	bnxt_dl_prealloc_quic_kid_param_set(dl, id, ctx)
...
#define bnxt_dl_enable_vf_roce_param_set(dl, id, ctx, extack)	\
	bnxt_dl_enable_vf_roce_param_set(dl, id, ctx)

#define bnxt_dl_disable_vf_roce_param_set(dl, id, ctx, extack)	\
	bnxt_dl_disable_vf_roce_param_set(dl, id, ctx)

#define bnxt_dl_vf_roce_set(bp, vf_idx, enable, extack)	\
	bnxt_dl_vf_roce_set(bp, vf_idx, enable)

#define bnxt_dl_ulp_num_rings_param_set(dl, id, ctx, extack)	\
	bnxt_dl_ulp_num_rings_param_set(dl, id, ctx)

#define bnxt_dl_ulp_num_vnics_param_set(dl, id, ctx, extack)	\
	bnxt_dl_ulp_num_vnics_param_set(dl, id, ctx)
#else
...
#else
#define netif_set_tso_max_size(d, s)
#endif
...
#ifndef HAVE_PCI_IOV_VF_ID
static inline int pci_iov_vf_id(struct pci_dev *dev)
{
#ifdef CONFIG_PCI_IOV
	u16 num_vfs, vf_offset, vf_stride;
	u16 dev_id, pf_dev_id;
	struct pci_dev *pf;
	int pos, vf_id;
	...
	pf = dev->physfn;
	pos = pci_find_ext_capability(pf, PCI_EXT_CAP_ID_SRIOV);
	if (!pos)
		return -EINVAL;

	pci_read_config_word(pf, pos + PCI_SRIOV_NUM_VF, &num_vfs);
	pci_read_config_word(pf, pos + PCI_SRIOV_VF_OFFSET, &vf_offset);
	pci_read_config_word(pf, pos + PCI_SRIOV_VF_STRIDE, &vf_stride);

	dev_id = PCI_DEVID(dev->bus->number, dev->devfn);
	pf_dev_id = PCI_DEVID(pf->bus->number, pf->devfn);
	vf_id = (dev_id - pf_dev_id - vf_offset) / vf_stride;
...
#endif /* !HAVE_PCI_IOV_VF_ID */

#ifndef HAVE_XDP_UPDATE_SKB_FRAGS_INFO

#define xdp_buff_get_skb_flags(xdp) xdp_buff_is_frag_pfmemalloc(xdp)

#define xdp_update_skb_frags_info(skb, nr_frags, size, truesize, xdp_flags) \
	xdp_update_skb_shared_info(skb, nr_frags, size, truesize, xdp_flags)
#endif

#ifndef HAVE_INET6_LOOKUP_WITH_HASHINFO

#define __inet6_lookup_established(net, hashinfo, saddr, sport, daddr, dport, ifindex, sdif) \
	__inet6_lookup_established(net, saddr, sport, daddr, dport, ifindex, sdif)
#endif

#ifndef HAVE_INET_LOOKUP_WITH_HASHINFO

#define inet_lookup_established(net, hashinfo, saddr, sport, daddr, dport, ifindex) \
	inet_lookup_established(net, saddr, sport, daddr, dport, ifindex)

#endif

#ifndef HAVE_NETDEV_QUEUE_CONFIG

#define bnxt_queue_mem_alloc(dev, qcfg, qmem, idx) \
	bnxt_queue_mem_alloc(dev, qmem, idx)

#define bnxt_queue_start(dev, qcfg, qmem, idx) \
	bnxt_queue_start(dev, qmem, idx)
