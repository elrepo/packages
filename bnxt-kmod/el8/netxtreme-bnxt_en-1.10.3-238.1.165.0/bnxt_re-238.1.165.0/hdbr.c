...
static void hdbr_dealloc_page(struct bnxt_re_dev *rdev, struct hdbr_pg *pg, int group)
{
	...
	if (!wk) {
		/* Cannot defer; recycle synchronously instead of leaking */
		mutex_lock(&rdev->hdbr_fpg_lock);
		list_add_tail(&pg->pg_node, &rdev->hdbr_fpgs);
		mutex_unlock(&rdev->hdbr_fpg_lock);
		return;
	}
			    ...
			    struct bnxt_re_ucontext *cntx, struct bnxt_re_srq_resp *resp)
{
	...
	if (resp) {
		resp->comp_mask |= BNXT_RE_SRQ_HDBR_MMAP_KEY;
			   ...
			   struct bnxt_re_pd *pd, struct bnxt_re_qp_resp *resp)
{
	...
	/* sq */
	if (resp) {
		mmap_key = &resp->hdbr_kaddr_sq;
		resp->comp_mask = BNXT_RE_QP_RESP_MASK_HDBR_KADDR_QP;
	}
	dbinfo = &qp->qplib_qp.sq.dbinfo;
	...
	if (resp) {
		...
		resp->comp_mask |= BNXT_RE_QP_RESP_MASK_HDBR_DEBUG_TRACE;
	}
