 ...
 *
 *  Copyright (C) 2013-2025 Philip J Perry <phil@elrepo.org>
 *
...
#define NVIDIA_VERSION		"570.172.08"
...
#define KMOD_NVIDIA_71XX	""	/* No longer supported on RHEL */
#elif RHEL_MAJOR == 9
#define KMOD_NVIDIA		"kmod-nvidia"
#define KMOD_NVIDIA_OPEN	"kmod-nvidia-open"
 ...
 */
#define XORG_ABI_CURRENT	25	/* 570.xxx.xx; Xorg 21.1 */
...
#define XORG_ABI_340XX		24	/* 340.108; Xorg 1.20 */
#define XORG_ABI_304XX		23	/* 304.137; Xorg 1.19 */
#define XORG_ABI_173XX		15	/* 173.14.39; Xorg 1.15 */
#define XORG_ABI_96XX		10	/* 96.43.23; Xorg 1.10 */
#define XORG_ABI_71XX		0	/* 71.86.15; Xorg 6.7 */
...
static void usage(void)
{
	...
	printf("%2d: No supported devices found\n", NVIDIA_NONE);
	printf("%2d: Device supported by the current %s NVIDIA open driver\n", NVIDIA_CURRENT_OPEN, NVIDIA_VERSION);
	printf("%2d: Device supported by the current %s NVIDIA driver\n", NVIDIA_CURRENT, NVIDIA_VERSION);
	printf("%2d: Device supported by the legacy  71.xx NVIDIA driver\n", NVIDIA_LEGACY_71XX);
	printf("%2d: Device supported by the legacy  96.xx NVIDIA driver\n", NVIDIA_LEGACY_96XX);
	printf("%2d: Device supported by the legacy 173.xx NVIDIA driver\n", NVIDIA_LEGACY_173XX);
	printf("%2d: Device supported by the legacy 304.xx NVIDIA driver\n", NVIDIA_LEGACY_304XX);
	printf("%2d: Device supported by the legacy 340.xx NVIDIA driver\n", NVIDIA_LEGACY_340XX);
...
static void list_all_nvidia_devices(void)
{
...
#ifndef __i386__
	printf("\n*** Devices supported by the current %s NVIDIA open driver %s ***\n\n",
		NVIDIA_VERSION, KMOD_NVIDIA_OPEN);
	for (i = 0; i < ARRAY_SIZE(nv_current_open_pci_ids); i++) {
		name = pci_lookup_name(pacc, namebuf, sizeof(namebuf),
			PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE,
			PCI_VENDOR_ID_NVIDIA, nv_current_open_pci_ids[i]);

		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_current_open_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_470xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_470xx_pci_ids[i], name);
	}
#endif /* __i386__ */
	...
	for (i = 0; i < ARRAY_SIZE(nv_390xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_390xx_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_367xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_367xx_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_340xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_340xx_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_304xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_304xx_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_173xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_173xx_pci_ids[i], name);
	}
	...
	for (i = 0; i < ARRAY_SIZE(nv_96xx_pci_ids); i++) {
		...
		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_96xx_pci_ids[i], name);
	}

	printf("\n*** Devices supported by the legacy 71.xx NVIDIA driver %s ***\n\n",
		KMOD_NVIDIA_71XX);
	for (i = 0; i < ARRAY_SIZE(nv_71xx_pci_ids); i++) {
		name = pci_lookup_name(pacc, namebuf, sizeof(namebuf),
			PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE,
			PCI_VENDOR_ID_NVIDIA, nv_71xx_pci_ids[i]);

		printf("[%04x:%04x] %s\n", PCI_VENDOR_ID_NVIDIA, nv_71xx_pci_ids[i], name);
	}
}
...
static int nv_lookup_device_id(u_int16_t device_id)
{
...
#ifndef __i386__
	/** Find devices supported by the current open driver **/
	for (i = 0; i < ARRAY_SIZE(nv_current_open_pci_ids); i++) {
		if (device_id == nv_current_open_pci_ids[i]) {
			if (opt_verbose) {
				printf("This device requires the current %s NVIDIA "
					"driver %s\n", NVIDIA_VERSION, KMOD_NVIDIA_OPEN);
			}
			return NVIDIA_CURRENT_OPEN;
		}
	}
	...
	/** Find devices supported by the 71xx legacy driver **/
	for (i = 0; i < ARRAY_SIZE(nv_71xx_pci_ids); i++) {
		if (device_id == nv_71xx_pci_ids[i]) {
			if (opt_verbose) {
				printf("This device requires the legacy 71.xx NVIDIA "
					"driver %s\n", KMOD_NVIDIA_71XX);
			}
			return NVIDIA_LEGACY_71XX;
		}
	}
...
{
	if (ret == NVIDIA_CURRENT_OPEN) {
		printf("%s\n", KMOD_NVIDIA_OPEN);
		return 0;
	}
	else if (ret == NVIDIA_CURRENT) {
		printf("%s\n", KMOD_NVIDIA);
		return 0;
	}
	...
	else if (ret == NVIDIA_LEGACY_71XX) {
		printf("%s\n", KMOD_NVIDIA_71XX);
		return 0;
	}
	else {
...
static bool check_xorg_abi_compat(int driver)
{
	...
	if (abi > 0) {
		if (driver == NVIDIA_CURRENT_OPEN && abi <= XORG_ABI_CURRENT )
			...
			return 1;
		else if (driver == NVIDIA_LEGACY_71XX && abi <= XORG_ABI_71XX )
			return 1;
		else
			return 0;
	}
