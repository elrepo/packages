...
static void r8127_dash_set_ipc2_reg_bit(struct rtl8127_private *tp, unsigned long reg, u32 mask)
{
        RTL_DASH_IPC2_W32(tp, reg, RTL_DASH_IPC2_R32(tp, reg) | mask);
}

/*
static void r8127_dash_clear_ipc2_reg_bit(struct rtl8127_private *tp, unsigned long reg, u32 mask)
{
        RTL_DASH_IPC2_W32(tp, reg, RTL_DASH_IPC2_R32(tp, reg) & ~mask);
}
*/

static void r8127_write_ipc2_tx_ack(struct rtl8127_private *tp)
{
        if (!tp->DASH)
                return;
        ...
        r8127_dash_set_ipc2_reg_bit(tp, IPC2_TX_SET_REG, IPC2_TX_ACK_BIT);
}

static void r8127_write_ipc2_tx_polling(struct rtl8127_private *tp)
{
        if (!tp->DASH)
                return;
        ...
        r8127_dash_set_ipc2_reg_bit(tp, IPC2_TX_SET_REG, IPC2_TX_SEND_BIT);
}

static unsigned long
r8127_get_ipc2_rx_buffer(struct rtl8127_private *tp)
{
        if (HW_DASH_SUPPORT_IPC2(tp))
...
static u8 rtl8127_copy_from_ipc2(struct rtl8127_private *tp, u8 *dest, u32 len)
{
        unsigned long const data_reg = r8127_get_ipc2_rx_buffer(tp);
        u32 offset = 0;
        u32 *pDword;
        u8 *pByte;
...
static void RecvFromDashFwComplete(struct rtl8127_private *tp)
{
        if (!tp->DASH)
                return;
        ...
        if (tp->DashReqRegValue == DASH_OOB_HDR_TYPE_REQ) { //rok
                ...
                /* copy data */
                rtl8127_copy_from_ipc2(tp,
                                       tp->AfterRecvFromFwBuf,
                                       tp->AfterRecvFromFwBufLen);

                r8127_write_ipc2_tx_ack(tp);
...
static unsigned long r8127_get_ipc2_tx_buffer(struct rtl8127_private *tp)
{
        if (HW_DASH_SUPPORT_IPC2(tp))
...
static u32 rtl8127_copy_to_ipc2(struct rtl8127_private *tp, u8 *src, u32 len)
{
        unsigned long const data_reg = r8127_get_ipc2_tx_buffer(tp);
        u32 offset = 0;
        u32 *pDword;
        u8 *pByte;
...
static int SendToDashFw(struct rtl8127_private *tp, u8 *src, u16 len)
{
        POSOOBHdr pOobHdr;
        int rc = -1;
        ...
        rtl8127_copy_to_ipc2(tp, src, len);

        r8127_write_ipc2_tx_polling(tp);
...
static u32 rtl8127_get_ipc2_isr(struct rtl8127_private *tp)
{
        u32 isr = 0;
...
static void rtl8127_set_ipc2_isr(struct rtl8127_private *tp, u32 val)
{
        if (FALSE == HW_DASH_SUPPORT_IPC2(tp))
                return;
...
{
        rtl8127_set_ipc2_isr(tp, rtl8127_get_ipc2_isr(tp));
}
...
void rtl8127_set_ipc2_soc_imr_bit(struct rtl8127_private *tp, u32 mask)
{
        ...
        RTL_W32(tp, RISC_IMR_8127AP, RTL_R16(tp, RISC_IMR_8127AP) | mask);
}
...
bool rtl8127_check_dash_interrupt(struct rtl8127_private *tp)
{
        ...
        isr = rtl8127_get_ipc2_isr(tp);

        if (isr & (IPC2_RX_ROK_BIT | IPC2_RX_ACK_BIT)) {
                set_bit(R8127_RCV_REQ_DASH_OK, tp->dash_req_flags);
...
void rtl8127_handle_dash_interrupt(struct net_device *dev)
{
        ...
        if (test_and_clear_bit(R8127_RCV_REQ_DASH_OK, tp->dash_req_flags))
