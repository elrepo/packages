...
static const struct {
        ...
        /* PCI-E devices. */
        [CFG_METHOD_1] = {"RTL8127A",                           },
        [CFG_METHOD_2] = {"RTL8127A",		FIRMWARE_8127A_1},
...
static u32 rtl8127_pci_config_read(struct rtl8127_private *tp, u32 addr);
static void rtl8127_pci_config_write(struct rtl8127_private *tp, u32 addr, u32 value);
...
static void rtl8127_sds_sfp_cap_task(void *_data);
#else
...
static void rtl8127_sds_sfp_cap_task(struct work_struct *work);
#endif
...
static u16 _rtl8127_read_thermal_sensor(struct rtl8127_private *tp)
{
        u16 ts_digout;
...
static int rtl8127_read_thermal_sensor(struct rtl8127_private *tp)
{
        int tmp;

        tmp = _rtl8127_read_thermal_sensor(tp);
        if (tmp > 512)
...
static int rtl8127_cel_to_fah(int cel)
{
        return (cel * 9 / 5) + 32;
}
...
static int proc_get_driver_variable(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "HwSerDesSfpCap\t0x%x\n", tp->HwSerDesSfpCap);
...
static int proc_get_pci_registers(struct seq_file *m, void *v)
{
        ...
        for (n = 0; n < max;) {
                ...
                for (i = 0; i < 4 && n < max; i++, n+=4) {
                        dword_rd = rtl8127_pci_config_read(tp, n);
                        seq_printf(m, "%08x ", dword_rd);
                }
        }
...
static int proc_get_temperature(struct seq_file *m, void *v)
{
        ...
        cel = rtl8127_read_thermal_sensor(tp);
...
static u32
rtl8127_pci_config_read(struct rtl8127_private *tp,
                        u32 addr)
{
        struct pci_dev *pdev = tp->pci_dev;
        u32 val;
        ...
        if (pdev->cfg_size > (addr + 3) &&
            pci_read_config_dword(pdev, addr, &val) == PCIBIOS_SUCCESSFUL)
                ...
                return rtl8127_csi_read(tp, addr);
}

static void
rtl8127_pci_config_write(struct rtl8127_private *tp,
                         u32 addr,
                         u32 value)
{
        struct pci_dev *pdev = tp->pci_dev;
        ...
        if (pdev->cfg_size > (addr + 3) &&
            pci_write_config_dword(pdev, addr, value) == PCIBIOS_SUCCESSFUL)
                ...
                rtl8127_csi_write(tp, addr, value);
}
...
rtl8127_set_pci_99_exit_driver_para(struct net_device *dev)
{
        ...
        if (test_bit(R8127_FLAG_SUSPEND | R8127_FLAG_SHUTDOWN, tp->task_flags))
                             ...
                             )
{
        ...
        if ((supported & SUPPORTED_Autoneg) && (bmcr & BMCR_ANENABLE))
                autoneg = AUTONEG_ENABLE;
        else
        ...
        advertising |= HW_FIBER_MODE_ENABLED(tp) ? ADVERTISED_FIBRE :ADVERTISED_TP;
        ...
        cmd->speed = speed;
        cmd->duplex = duplex;
        cmd->port = HW_FIBER_MODE_ENABLED(tp) ? PORT_FIBRE :PORT_TP;
        ...
        cmd->base.port = HW_FIBER_MODE_ENABLED(tp) ? PORT_FIBRE :PORT_TP;
                          ...
                          u64 *data)
{
        ...
        counters = tp->tally_vaddr;
        paddr = tp->tally_paddr;
        if (!counters || test_bit(R8127_FLAG_SHUTDOWN, tp->task_flags))
                return;
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,0,0)
static void
rtl8127_set_output_gpio(struct rtl8127_private *tp, u16 led_sel, bool enable)
{
        if (enable) {
                /* enable LED output */
                rtl8127_set_mac_ocp_bit(tp, 0xDC4C, led_sel);
                rtl8127_set_mac_ocp_bit(tp, 0xDC52, led_sel);
                rtl8127_set_mac_ocp_bit(tp, 0xDC5E, led_sel);
                rtl8127_set_mac_ocp_bit(tp, 0xDC46, led_sel);
        } else {
                /* disable GPIO Function */
                rtl8127_clear_mac_ocp_bit(tp, 0xDC5E, led_sel);
                rtl8127_clear_mac_ocp_bit(tp, 0xDC46, led_sel);
        }
}

static void
rtl8127_set_led_on(struct rtl8127_private *tp, u16 led_sel, bool on)
{
        if (on)
                rtl8127_set_mac_ocp_bit(tp, 0xDC52, led_sel);
        else
                rtl8127_clear_mac_ocp_bit(tp, 0xDC52, led_sel);
}

static int
rtl8127_set_phys_id(struct net_device *dev,
                    enum ethtool_phys_id_state state)
{
        struct rtl8127_private *tp = netdev_priv(dev);
        const u16 led_sel = 0x01a4;

        switch (state) {
        case ETHTOOL_ID_ACTIVE:
                rtl8127_set_output_gpio(tp, led_sel, true);
                return 2; /* cycle on/off twice per second */
        case ETHTOOL_ID_ON:
                rtl8127_set_led_on(tp, led_sel, true);
                break;
        case ETHTOOL_ID_OFF:
                rtl8127_set_led_on(tp, led_sel, false);
                break;
        case ETHTOOL_ID_INACTIVE:
                rtl8127_set_output_gpio(tp, led_sel, false);
                break;
        default:
                break;
        }
...
static int rtl_get_eeprom(struct net_device *dev, struct ethtool_eeprom *eeprom, u8 *buf)
{
        ...
        for (i=start_w; i<=end_w; i++) {
                ...
                eeprom_buff[i-start_w] = rtl8127_pci_config_read(tp, VPD_data);
        }
        rtl8127_disable_cfg9346_write(tp);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,0,0)
        .set_phys_id        = rtl8127_set_phys_id,
#endif //LINUX_VERSION_CODE < KERNEL_VERSION(3,0,0)
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,33)
...
static bool
rtl8127_wait_phy_state_ready(struct rtl8127_private *tp, u16 state,
                             u32 usec)
{
        u16 tmp_state;
        u32 wait_cnt;
        ...
        i = 0;
        do {
                tmp_state = rtl8127_get_phy_state(tp);
                mdelay(1);
                i++;
...
rtl8127_set_mac_mcu_8127a_1(struct net_device *dev)
{
        ...
        //rtl8127_mac_ocp_write(tp, 0xFC38, 0x1A84);
        rtl8127_mac_ocp_write(tp, 0xFC3C, 0x1598);

        rtl8127_mac_ocp_write(tp, 0xFC48, 0x04FF);
}
...
rtl8127_hw_init(struct net_device *dev)
{
        ...
        csi_tmp |= BIT_20;
        rtl8127_pci_config_write(tp, 0x108, csi_tmp);
...
rtl8127_hw_phy_config_8127a_1(struct net_device *dev)
{
        ...
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x83A5);
        rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xB87E,
                                              0xFF00,
                                              0x0400);
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x83A6);
        rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xB87E,
                                              0xFF00,
                                              0x0400);
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x83A7);
        rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xB87E,
                                              0xFF00,
                                              0x0400);
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x83A8);
        rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xB87E,
                                              0xFF00,
                                              0x0400);
        ...
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xB87E, 0xF4FF);
        ...
        rtl8127_set_eth_phy_ocp_bit(tp, 0xA442, BIT_7);
        rtl8127_clear_eth_phy_ocp_bit(tp, 0xA430, BIT_12);


        if (aspm && !HW_FIBER_MODE_ENABLED(tp) &&
...
rtl8127_setup_interrupt_mask(struct rtl8127_private *tp)
{
        ...
        if (tp->HwCurrIsrVer == 6) {
                tp->intr_mask = ISRIMR_V6_LINKCHG;
                for (i = 0; i < tp->num_tx_rings; i++)
                        tp->intr_mask |= ISRIMR_V6_TOK_Q0 << i;
                for (i = 0; i < tp->num_rx_rings; i++)
...
static void
rtl8127_backup_led_select(struct rtl8127_private *tp)
{
...
static void
rtl8127_restore_led_select(struct rtl8127_private *tp)
{
...
static bool
_rtl8127_backup_phy_fuse_dout_v4(struct rtl8127_private *tp)
{
        u16 i;

        for (i = 0; i < R8127_PHY_FUSE_DOUT_NUM; i++) {
                rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                                      0xA460,
                                                      0x001F,
                                                      i);
                tp->BackupPhyFuseDout[i] = rtl8127_mdio_direct_read_phy_ocp(tp, 0xA462);
        }

        if (tp->HwSuppEsdVer == 6) {
...
static bool
rtl8127_backup_phy_fuse_dout(struct rtl8127_private *tp)
{
        if (tp->HwSuppEsdVer == 6)
                return _rtl8127_backup_phy_fuse_dout_v4(tp);
        else
                return FALSE;
}

static void
_rtl8127_restore_phy_fuse_dout_v4(struct rtl8127_private *tp)
{
        u16 i;
        ...
        for (i = 0; i < R8127_PHY_FUSE_DOUT_NUM; i++) {
                ...
                rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                                      ...
                                                      0x001F,
                                                      i);
                rtl8127_mdio_direct_write_phy_ocp(tp, 0xA462, tp->BackupPhyFuseDout[i]);
        }
}

static void
rtl8127_restore_phy_fuse_dout(struct rtl8127_private *tp)
{
        if (tp->HwSuppEsdVer == 6)
                _rtl8127_restore_phy_fuse_dout_v4(tp);
        else
                return;
}
...
rtl8127_init_software_variable(struct net_device *dev)
{
        ...
        if (rtl8127_backup_phy_fuse_dout(tp))
        ...
        tp->HwSuppNumTxQueues = R8127_TX_QUEUE_NUM;
        tp->HwSuppNumRxQueues = R8127_RX_QUEUE_NUM;
                                  ...
                                  void __user *data, int cmd)
{
        ...
        case SIOCDEVPRIVATE_RTLDASH:
                ...
                ret = rtl8127_dash_ioctl(dev, &ifrdata);
                break;
#endif
        ...
        case SIOCDEVPRIVATE_RTLREALWOW:
                ...
                ret = rtl8127_realwow_ioctl(dev, &ifrdata);
                break;
#endif
        ...
        case SIOCRTLTOOL:
                ...
                ret = rtl8127_tool_ioctl(tp, &ifrdata);
                break;
...
static bool
rtl8127_test_phy_ocp_v6(struct rtl8127_private *tp)
{
        bool restore = FALSE;
        u8 phy_fatal_err;
        u32 wait_cnt;
        ...
        phy_fatal_err = rtl8127_mdio_direct_read_phy_ocp(tp, 0xB98C);

        if (phy_fatal_err == 0)
                goto exit;

        rtl8127_set_eth_phy_ocp_bit(tp, 0xBC4C, 0x0003);
        rtl8127_set_eth_phy_ocp_bit(tp, 0xBC0E, 0x00C0);
        rtl8127_set_eth_phy_ocp_bit(tp, 0xBC02, 0x0003);
        rtl8127_set_eth_phy_ocp_bit(tp, 0xBC04, 0x0003);
        rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                              0xC400,
                                              0x001F,
                                              0x0004);
        rtl8127_set_eth_phy_ocp_bit(tp, 0xC400, BIT_8);

        rtl8127_set_eth_phy_ocp_bit(tp, 0xC418, BIT_0);

        rtl8127_wait_phy_state_ready(tp, HW_PHY_STATUS_INI, 5000000);

        rtl8127_set_eth_phy_ocp_bit(tp, 0xBC04, 0x0003);
        udelay(10);
        rtl8127_clear_eth_phy_ocp_bit(tp, 0xBC04, 0x0003);

        wait_cnt = 0;
        while ((rtl8127_mdio_direct_read_phy_ocp(tp, 0xC404) & BIT_0) && wait_cnt < 50) {
                mdelay(100);
                wait_cnt++;
        }

        rtl8127_restore_phy_fuse_dout(tp);

        if (tp->pci_dev->revision != 0x01) {
                rtl8127_clear_eth_phy_ocp_bit(tp, 0xA448, BIT_10);
                rtl8127_clear_eth_phy_ocp_bit(tp, 0xA586, BIT_10);
                rtl8127_clear_and_set_eth_phy_ocp_bit(tp,
                                                      0xBFA0,
                                                      0x001F,
                                                      0x000C);
        ...
        rtl8127_set_eth_phy_ocp_bit(tp, 0xA468, BIT_0);

        rtl8127_clear_phy_ups_reg(tp->dev);

        rtl8127_wait_phy_state_ready(tp, HW_PHY_STATUS_LAN_ON, 100000);

        rtl8127_mdio_direct_write_phy_ocp(tp, 0xA436, 0x801C);
        rtl8127_mdio_direct_write_phy_ocp(tp, 0xA438, phy_fatal_err);

        rtl8127_restore_led_select(tp);
...
static bool
rtl8127_test_phy_ocp(struct rtl8127_private *tp)
{
        unsigned long flags;
        bool reset = false;
        ...
        case 6:
                reset = rtl8127_test_phy_ocp_v6(tp);
                break;
        default:
                goto unlock;
        }
...
rtl8127_esd_checker(struct rtl8127_private *tp)
{
        ...
        if (tp->HwPcieSNOffset > 0) {
                pci_sn_l = rtl8127_pci_config_read(tp, tp->HwPcieSNOffset);
                if (pci_sn_l != tp->pci_cfg_space.pci_sn_l) {
                        ...
                        rtl8127_pci_config_write(tp, tp->HwPcieSNOffset, tp->pci_cfg_space.pci_sn_l);
                        tp->esd_flag |= BIT_13;
                }

                pci_sn_h = rtl8127_pci_config_read(tp, tp->HwPcieSNOffset + 4);
                if (pci_sn_h != tp->pci_cfg_space.pci_sn_h) {
                        ...
                        rtl8127_pci_config_write(tp, tp->HwPcieSNOffset + 4, tp->pci_cfg_space.pci_sn_h);
                        tp->esd_flag |= BIT_14;
                }
        }

        if (tp->TestPhyOcpReg && rtl8127_test_phy_ocp(tp))
                tp->esd_flag |= BIT_15;
...
static void
rtl8127_sds_sfp_cap_checker(struct rtl8127_private *tp)
{
        unsigned long flags;
        ...
        /* SFP is connected */
        if (!rtl8127_sds_sfp_connected_8127(tp))
                goto unlock;

        u32 currCap = rtl8127_sds_If_i2c_is_10g_sfp_cap(tp) ? 10000 : 1000;

        currCap = min(currCap, tp->speed);
        if (tp->HwSerDesSfpCap != currCap)
                _rtl8127_set_sds_phy_caps_8127(tp, currCap);
...
int rtl8127_open(struct net_device *dev)
{
        ...
        rtl8127_schedule_sds_sfp_cap_work(tp);
...
_rtl8127_set_l1_l0s_entry_latency(struct rtl8127_private *tp, u8 setting)
{
        ...
        csi_tmp = rtl8127_pci_config_read(tp, 0x70c) & 0xc0ffffff;
        rtl8127_pci_config_write(tp, 0x70c, csi_tmp | temp);
}
...
{
        rtl8127_pci_config_write(tp, 0x890, rtl8127_pci_config_read(tp, 0x890) & ~BIT(0));
}
...
rtl8127_hw_config(struct net_device *dev)
{
        ...
        if (!tp->pci_cfg_is_read) {
                ...
                if (tp->HwPcieSNOffset > 0) {
                        tp->pci_cfg_space.pci_sn_l = rtl8127_pci_config_read(tp, tp->HwPcieSNOffset);
                        tp->pci_cfg_space.pci_sn_h = rtl8127_pci_config_read(tp, tp->HwPcieSNOffset + 4);
                }
...
#define rtl8127_cancel_schedule_sds_sfp_cap_work(a)
...
static void rtl8127_cancel_schedule_sds_sfp_cap_work(struct rtl8127_private *tp)
{
        struct work_struct *work = &tp->sds_sfp_cap_task.work;

        if (!work->func)
                return;

        cancel_delayed_work_sync(&tp->sds_sfp_cap_task);
}
#endif
...
static void rtl8127_init_all_schedule_work(struct rtl8127_private *tp)
{
        ...
        INIT_WORK(&tp->sds_sfp_cap_task, rtl8127_sds_sfp_cap_task, dev);
#else
        ...
        INIT_DELAYED_WORK(&tp->sds_sfp_cap_task, rtl8127_sds_sfp_cap_task);
#endif
}
...
static void rtl8127_cancel_all_schedule_work(struct rtl8127_private *tp)
{
        ...
        rtl8127_cancel_schedule_sds_sfp_cap_work(tp);
}
...
{
        struct rtl8127_private *tp =
                container_of(work, struct rtl8127_private, sds_sfp_cap_task.work);
        struct net_device *dev = tp->dev;
#endif
        rtnl_lock();
        ...
        if (!netif_running(dev) ||
            test_bit(R8127_FLAG_DOWN, tp->task_flags) ||
            !test_and_clear_bit(R8127_FLAG_TASK_SDS_SFP_CAP_PENDING, tp->task_flags))
                goto out_unlock;

        rtl8127_sds_sfp_cap_checker(tp);
...
static u32
rtl8127_get_skb_mss(struct sk_buff *skb)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18)
                 ...
                 unsigned short *gso_segs)
{
        ...
        if (dev->features & (NETIF_F_TSO | NETIF_F_TSO6)) {
                u32 mss = rtl8127_get_skb_mss(skb);
...
static netdev_tx_t
_rtl8127_start_xmit(struct sk_buff *skb,
                    struct net_device *dev)
{
        struct rtl8127_private *tp = netdev_priv(dev);
        unsigned int   bytecount;
...
static bool
rtl8127_tso_quirk_gso_check(struct net_device *dev, struct sk_buff *skb)
{
        if ((dev->features & (NETIF_F_TSO | NETIF_F_TSO6)) &&
            (rtl8127_get_skb_mss(skb) > MSS_MAX))
                return true;
        else
                return false;
}

static int rtl8127_tso_quirk(struct net_device *dev, struct sk_buff *skb)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18)
        return _rtl8127_start_xmit(skb, dev);
#else
        const u16 queue_mapping = skb_get_queue_mapping(skb);
        struct rtl8127_private *tp = netdev_priv(dev);
...
rtl8127_start_xmit(struct sk_buff *skb,
                   struct net_device *dev)
{
        if (rtl8127_tso_quirk_gso_check(dev, skb))
                return rtl8127_tso_quirk(dev, skb);
        else
                return _rtl8127_start_xmit(skb, dev);
}

static void
rtl8127_desc_quirk(struct rtl8127_private *tp)
{
                                 ...
                                 int budget)
{
        ...
        switch (tp->HwCurrIsrVer) {
        case 6:
                if (message_id > 7 && message_id < 16) {
                        u32 idx = message_id - 8;
                        count += rtl8127_tx_interrupt(&tp->tx_ring[idx],
                                                      budget);
                }
                break;
        default:
...
static void rtl8127_shutdown(struct pci_dev *pdev)
{
        ...
        set_bit(R8127_FLAG_SHUTDOWN, tp->task_flags);
...
#endif
{
        ...
        set_bit(R8127_FLAG_SUSPEND, tp->task_flags);
...
#endif
{
        ...
        clear_bit(R8127_FLAG_SUSPEND, tp->task_flags);
