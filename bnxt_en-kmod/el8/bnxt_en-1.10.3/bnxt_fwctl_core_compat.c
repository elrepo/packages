...
#ifndef HAVE_CLEANUP_KVFREE
static inline void __free_kvfree(void *T)
{
	if (!IS_ERR_OR_NULL(T))
		kvfree(T);
}
...
static int fwctl_cmd_rpc(struct fwctl_ucmd *ucmd)
{
	...
	out_len = cmd->out_len;
	void *outbuf __free(kvfree) = fwctl->ops->fw_rpc(ucmd->uctx,
