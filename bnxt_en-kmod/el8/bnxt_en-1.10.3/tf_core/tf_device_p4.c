 ...
 * @max_types:	Pointer to the MAX number of CFA resource types supported
 *
 * Returns
 *   - (0) if successful.
 *   - (-EINVAL) on failure.
 */
static int tf_dev_p4_get_max_types(struct tf *tfp, u16 *max_types)
{
	if (!max_types || !tfp)
		return -EINVAL;
 ...
 *   - (-EINVAL) on failure.
 */
static int tf_dev_p4_get_resource_str(struct tf *tfp, u16 resource_id,
 ...
 *   - (-EINVAL) on failure.
 */
static int tf_dev_p4_set_tcam_slice_info(struct tf *tfp,
 ...
 *
 * @tfp:	Pointer to TF handle
 * @slice_size:	Pointer to the WC TCAM slice size[out]
 * @num_slices_per_row:	Pointer to the WC TCAM row slice configuration[out]
 *
 * Returns
 *   - (0) if successful.
 *   - (-EINVAL) on failure.
 ...
 */
static int
tf_dev_p4_get_shared_tbl_increment(struct tf *tfp,
...
static int tf_dev_p4_word_align(u16 size)
{
	return TF_BITS2BYTES_WORD_ALIGN(size);
}
 ...
 */
static bool tf_dev_p4_is_sram_managed(struct tf *tfp,
 ...
 *   - (-EINVAL) on failure.
 */
static int tf_dev_p4_map_hcapi_caps(u64 hcapi_caps, u32 *ident_caps,
